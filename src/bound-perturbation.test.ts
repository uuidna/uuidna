...
test('CONTROL — a bound that is not load-bearing survives widening', () => {
  assert.equal(boundVerdict('((List.range 63).all (fun x => ((x - x % 9) % 9 == 0) && (x % 9 < 9)))'), 'not-load-bearing')
})
...
test('the census separates on a sample, and its parts sum to the whole', () => {
  ...
  assert.equal(c.notLoadBearing + c.loadBearing + c.baseUndecidable + c.undecidableWidened, c.bounded)
  // both classes must be NON-EMPTY, or the probe has collapsed to one answer and measures nothing
  assert.ok(c.notLoadBearing > 0, 'if nothing survives widening the probe is broken, not the ledger')
