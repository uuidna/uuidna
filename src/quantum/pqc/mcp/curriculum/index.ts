// quantum/pqc/mcp/curriculum — PQC posture as hosted MCP calls (symmetric present, hybrid slots named absent).
...
/** pqcCurriculum() → symmetric widths + profile + theorems; hybrid suite registered, KEM slots absent. */
export function pqcCurriculum(): PqcMcpCurriculum {
  const examples: AdvantageMcpExample[] = [
    ...
    example('pqc-label', 'grover_quadratic_bound', 'uuidna_crypto', {},
      ...
      'posture is PQC-adjacent: symmetric stack present, hybrid not deployable yet'),
    example('pqc-missing', 'grover_quadratic_bound', 'uuidna_crypto', {},
      [{ path: 'pqc.hybridDeployable', equals: false }],
      'ML-KEM / X25519 / ML-DSA slots are named but not wired — hybridDeployable is false'),
    ...
    example('gate-fabricated-kem', 'grover_quadratic_bound', 'uuidna_gate', {
      ...
      'fabricated ML-KEM theorem drains — the KEM is not sealed in the ledger'),
    ...
    example('suite-hybrid', 'grover_quadratic_bound', 'uuidna_crypto', {},
      [{ path: 'pqc.suites.1.id', equals: HYBRID_SUITE_ID }, { path: 'pqc.suites.1.present', equals: false }],
      `hybrid suite ${HYBRID_SUITE_ID} is registered but not present until KEM instruments land`),
  ]
...
export function renderPqcMcpMarkdown(cur = pqcCurriculum()): string {
  ...
  return [
    ...
    ...rows,
    '',
    `Curriculum receipt \`${cur.receipt}\`. PQC-adjacent ≠ PQC-enabled: ChaCha20-Poly1305 consumes derived subkeys; ML-KEM/ML-DSA are not on the wire yet.`,
  ].join('\n')
}
