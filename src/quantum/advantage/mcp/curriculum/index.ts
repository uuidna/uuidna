...
import { ADVANTAGE_MCP_ORIGIN } from '../wire/index.js'
...
/** advantageCurriculum() → the quantum-advantage curriculum as hosted MCP calls. Numbers from constructors. */
export function advantageCurriculum(): AdvantageMcpCurriculum {
  ...
  const examples: AdvantageMcpExample[] = [
    ...
    example('crypto-widths', 'sha256_grover_margin_is_the_address', 'uuidna_crypto', {},
      [
        ...
        { path: 'widths.shor.uuidFits', equals: shor.uuidFits },
    ...
    example('read-cost', 'n_qubit_dimension', 'uuidna_theorem', { key: 'n_qubit_dimension' },
      [{ path: 'key', equals: 'n_qubit_dimension' }, { path: 'verdict', equals: 'SEALED' }],
    ...
    example('gate-overclaim', 'n_qubit_dimension', 'uuidna_gate', { claim: ADVANTAGE_OVERCLAIM },
      [{ path: 'verdict', equals: 'VERIFIED' }, { path: 'binary', equals: 1 }],
      'uuidna_gate still VERIFIEDs a sealed citation; read backing.says — VERIFIED is not endorsement'),
    example('skill-quantum', 'n_qubit_dimension', 'uuidna_skill', { skill: 'quantum' },
      [{ path: 'skill', equals: 'quantum' }],
      'the quantum skill is one door (not one tool per theorem); lab.computation is state-vector'),
...
export function renderAdvantageMcpMarkdown(cur = advantageCurriculum()): string {
  ...
  return [
    `Quantum advantage at school is **${cur.examples.length} MCP calls** on [\`${cur.endpoint}\`](${cur.endpoint}) — one door each, constructor values, no wall-clock, no new theorem ([\`n_qubit_dimension\`](/theorem/n_qubit_dimension), [\`usable_gap_is_two_to_eighty\`](/theorem/usable_gap_is_two_to_eighty)).`,
    '',
