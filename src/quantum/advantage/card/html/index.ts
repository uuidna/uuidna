...
// Page-local metrics via pageAdvantageMetrics; global usable_gap_is_two_to_eighty as context.
// Exact-integer bar widths only (no float log; exact integer bit-length only — eslint no-float-math).
import { handleOf } from '../../../../handle.js'
import { pageAdvantageMetrics } from '../../page/metrics/index.js'
...
/**
 * quantumAdvantageCardHtml({ address?, handle?, label?, heartbeats?, objectKind? }) →
 * metrics-defined advantage block for ANY card — page-local first, capacity as context.
 */
export function quantumAdvantageCardHtml(opts: {
  address?: string
  handle?: string
  label?: string
  ...
  const page = pageAdvantageMetrics({
    address: opts.address,
    handle: opts.handle,
    label: opts.label,
    heartbeats: opts.heartbeats,
    objectKind: opts.objectKind || 'card',
    maxHeartbeats: opts.maxHeartbeats,
  })
  const handle = page.handle || (opts.address ? handleOf(opts.address) : '')
  const label = opts.label ? escape(opts.label) : ''
  const w128 = barW(USABLE)
  const w48 = barW(REPORTED)
  const pageStats =
    (page.heartbeats != null
      ? `<span><b>${page.heartbeats}</b> hb</span>` +
        `<span><b>10^${page.heartbeatDecade}</b> decade</span>`
      : '') +
    (page.residue != null ? `<span><b>${page.residue}</b> ℤ/9</span>` : '') +
    (handle ? `<span><b>${escape(handle)}</b> handle</span>` : '')
  return (
    `<div class="qa-card-metrics" data-slot="quantum-advantage" data-page-handle="${escape(handle)}" data-heartbeats="${page.heartbeats ?? ''}" aria-label="Page metrics + usable-capacity gap (usable_gap_is_two_to_eighty)">` +
    `<div class="qa-card-head"><span class="qa-card-badge">page</span>` +
    ...
    `</div>` +
    `<div class="qa-card-stats">${pageStats || `<span><b>—</b> no local cost</span>`}</div>` +
    `<svg class="qa-card-svg" viewBox="0 0 200 36" role="img" aria-label="Usable capacity context 2^128 vs reported 48 logical">` +
    `<title>context usable 2^${USABLE} vs reported ${REPORTED} · gap 2^${GAP}</title>` +
