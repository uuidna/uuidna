...
import {
  ...
  renderRemainingAlpine, type PackageCheck,
} from './index.js'
import { planAlpineRuns } from '../../../os/runtime/index.js'
import { catalogue, cataloguePackage, isAlpineDistroPackage, providedCommands } from '../catalogue/index.js'
import { UUID_HEXBITS, UUID_BITS, nativeBitWidths } from '../../../hexbit/index.js'
import { theoremByKey } from '../../../index.js'
import { fresh, exec } from '../harness/index.js'
...
test('busybox — the toolbox package — is INSTALLED, man-tested, and planned', () => {
  ...
  assert.ok(spec)
  const t = testInstallPackage(spec)
  assert.equal(t.ok, true, t.checks.filter((c: PackageCheck) => !c.ok).map((c: PackageCheck) => `${c.check}: ${c.detail}`).join('; '))
