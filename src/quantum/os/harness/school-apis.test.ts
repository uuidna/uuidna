...
import { schoolApiRegistry, schoolApiFetch, probeSchoolApis, splitCsvLine, pickLang, SCHOOL_APIS, GISCO_VINTAGE,
  type SchoolApiAnswer } from '../../../index.js'
...
test('each fetched source is refused by name when its own required argument is missing', async () => {
  ...
  await assert.rejects(() => schoolApiFetch('cern-opendata'), /needs \{text\}/)
...
const fake = (by: Record<string, Partial<SchoolApiAnswer> | 'throw'>) =>
  async (source: string, _query?: unknown): Promise<SchoolApiAnswer> => {
    const r = by[source]
...
test('the heartbeat separates answering, empty, REFUSED and thrown — and never raises', async () => {
  const h = await probeSchoolApis(fake({
    ...
    cordis: { count: 1 }, 'cern-opendata': { count: 2 }, ted: { count: 2 },
  }))
  ...
  assert.equal(h.answering, 4)
  assert.equal(h.dark.length, 3)
})

test('cern-opendata is declared beside cordis as the HEP open-data door', () => {
  const cern = SCHOOL_APIS.find((s) => s.id === 'cern-opendata')
  const cordis = SCHOOL_APIS.find((s) => s.id === 'cordis')
  assert.ok(cern?.probe?.text, 'CERN Open Data must declare a known-good probe')
  assert.equal(cern?.kind, 'research')
  assert.ok(cordis, 'EU funding (cordis) and HEP data (cern-opendata) are separate doors')
})
...
test('the heartbeat receipt moves when a source goes dark, and is stable when nothing changes', async () => {
  const all = { esco: { count: 1 }, eurostat: { count: 1 }, gisco: { count: 1 },
    'data-europa': { count: 1 }, cordis: { count: 1 }, 'cern-opendata': { count: 1 }, ted: { count: 1 } }
