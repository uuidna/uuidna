...
test('home doors to /quantum; capacity table on /quantum and README only', () => {
  ...
  assert.match(home, /^## What you can do here$/m)
  assert.match(home, /^## For developers$/m)
  assert.ok(home.indexOf('## What you can do here') < home.indexOf('## For developers'), 'people first, setup second')
  assert.match(home, /<details>\s*<summary>The full live census<\/summary>/)
...
test('Clay is the visible test POC on home and README — computationally claimed', () => {
  ...
  assert.ok(census)
  assert.match(home, /\(\/theorem\/clay_gravity_equals_rosette\)/)
  // the hero cards speak in tasks; the Clay key rides the census below the fold
  const hero = homeHeroOf(census)
  assert.ok(!hero.features.some((f) => f.title.startsWith('clay_')))
  const readme = readFileSync(join(ROOT, 'README.md'), 'utf8')
