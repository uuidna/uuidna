// @non-harmonic: uuidnaOS school port — EU education APIs (network). One fetch cache shared with research and cern.
// school — THE EUROPEAN EDUCATION APIS BEHIND ONE DOOR, each one PROBED before it was wired.
//
...
import { fetchData, immutableText, immutableReads as osImmutableReads } from '../fetch/index.js'
...
/** immutableReads() → vintage-carrying GISCO URLs in the shared OS fetch cache, content-addressed. */
export function immutableReads(): { handle: string; url: string; bytes: number; door: string; hexbits: number[] }[] {
  return osImmutableReads().map(({ url, bytes }) => {
    const address = toUuid(url)
    const door = hexbitDoorOf(address)
    return { handle: door.handle, door: door.door, hexbits: door.hexbits, url, bytes }
  })
}
...
export const SCHOOL_APIS: readonly SchoolApi[] = [
  ...
  { id: 'cern-opendata', name: 'CERN Open Data — LHC experiment catalogue', base: CERN_OD,
    probe: { text: 'CMS Higgs', limit: 3 },
    kind: 'research', direction: 'fetched',
    serves: ['CMS/ATLAS/LHCb/ALICE dataset records', 'collision energy and type', 'file formats', 'record titles'],
    format: 'JSON (Invenio REST)', access: 'public, no key',
    honest: 'OPEN COLLIDER DATA — published dataset metadata from CERN experiments. Evidence of what the catalogue ' +
      'holds, never a physics proof and never a claim that uuidna analysed any collision. Complements CORDIS (EU ' +
      'funding records) with the HEP open-data door trials already reach through the research sweep.' },
...
export interface CernOpenDataRecord extends SchoolApiEvidence { id: string; title: string; experiment: string }

/** cernOpenDataSearch(text) → LHC open-data records via the uuidnaOS cern port. One network call. */
export async function cernOpenDataSearch(text: string, limit?: number): Promise<SchoolApiAnswer> {
  const n = limitOf(limit)
  const got = await fetchCernOpenData(text, n)
  const results: CernOpenDataRecord[] = got.records.map((r) => ({
    source: 'cern-opendata', address: r.address, id: r.id, title: r.title, experiment: r.experiment,
  }))
  return answer('cern-opendata', { text }, got.url, results, got.total > results.length, got.declined,
    got.declined ? got.note : '')
}
...
export async function schoolApiFetch(source: string, query: SchoolApiQuery = {}): Promise<SchoolApiAnswer> {
  ...
  if (source === 'cern-opendata') {
    if (!query.text) throw new Error('school-apis: cern-opendata needs {text} — the phrase to search LHC open data for')
    return cernOpenDataSearch(query.text, query.limit)
  }
