...
import { primeCatalogue, resetCatalogue, CATALOGUE_FILE } from '../catalogue/index.js'
import { packagePage, renderPackagePage } from './index.js'

// PRIMING REPLACES THE WORLD FOR THE WHOLE PROCESS: the suite runs every file in one, so each test that primes forgets it as it ends.
const prime = (t: { after: (fn: () => void) => void }): void => { primeCatalogue(readFileSync(join(ROOT, CATALOGUE_FILE), 'utf8')); t.after(() => resetCatalogue()) }

test('a published package computes its whole page from the mirror', (t) => {
  prime(t)
...
test('CONTROL — a name the catalogue does not publish is null, not an empty page', (t) => {
  prime(t)
...
test('the rendered page carries its payload as microdata, not as prose alone', (t) => {
  prime(t)
...
test('CONTROL — every value in the page is escaped, so metadata cannot become markup', (t) => {
  prime(t)
...
test('the tools are passed IN, so the page carries no opinion about which roster a host serves', (t) => {
  prime(t)
  const p = packagePage('busybox')!
