...
// actual bytes you hold with uuidna's OWN pure-TS SHA-256 (use only uuidna, never the host). So a deployment can PROVE
...
export interface DriverBundle {
  ...
  sha256: string        // Alpine's PUBLISHED digest for the bundle — the external anchor, never faked
...
const HONEST =
  ...
  'exactly which drivers you hold. uuidnaOS is only TypeScript. "Port" = port the INTEGRITY, never the runtime. Integrity, not execution.'
...
export function verifyDriverBundle(bytes: Uint8Array, bundle: DriverBundle): DriverCheck {
  ...
  return {
    ...
    honest: 'Exact-copy proof: uuidna re-hashed the driver bytes you hold with its own KAT-verified SHA-256 and matched ' +
      'them against the bundle you pinned. It proves provenance (you hold exactly these drivers), never that any module ran.',
  }
}
...
export async function fetchDriverLatest(arch = 'x86_64', branch = 'latest-stable'): Promise<DriverBundle | null> {
  try {
    ...
    // version/sha256/file; the document is content, not code. If the shape ever drifts we return null rather than guess.
