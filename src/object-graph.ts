...
//   axioms: wing defs cited · gravity · unbound · neighbour count
// Related:
...
/**
 * theoremGraph(t, all, bySkill, byPrin, legsRow, axiomHolds, relatedMaps?, axioms?)
 * → full related-object graph for ObjectCrosslinks + stock pager fields.
 */
export function theoremGraph(t, all, bySkill, byPrin, legsRow, axiomHolds, relatedMaps, axioms, face) {
  ...
  return {
    ...
    dependsOn: axioms?.dependsOn || [],
    wingDefs: axioms?.wingDefs || [],
    unusedDefs: axioms?.unusedDefs || [],
    depCount: axioms?.depCount ?? 0,
    gravity: axioms?.gravity ?? 0,
    unbound: !!axioms?.unbound,
    neighbourCount: axioms?.neighbourCount ?? 0,
    file: t.file,
    address: t.address,
    ...
    channel: (() => {
      try {
        const c = channelAudit(t.address)
        return { handle: c.handle, trinities: [...c.trinities], tail: c.tail, torusHome: c.torusHome }
      } catch { return null }
    })(),
...
export function publicationGraph(p, pubs, byKey, rich, seals) {
  ...
  const priorArt = rich?.priorArt
    ...
    : null
  const addr = p.address || p.receipt || ''
  let channel = null
  if (addr) {
    try {
      const c = channelAudit(addr)
      channel = { handle: c.handle, trinities: [...c.trinities], tail: c.tail, torusHome: c.torusHome }
    } catch { /* non-uuid */ }
  }
  return {
