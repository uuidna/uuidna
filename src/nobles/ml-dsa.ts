...
import { abool } from './curve-utils.js';
import { iabs, idiv } from './imath.js';
import { shake256 } from './sha3.js';
import type { CHash } from './hash-utils.js';
import { genCrystals, type XOF, XOF128, XOF256 } from './crystals.js';
import {
  abytes,
  type BytesCoderLen,
  checkHash,
...
const GAMMA2_1 = idiv(Q - 1, 88) | 0;
const GAMMA2_2 = idiv(Q - 1, 32) | 0;
...
const polyChknorm = (p_: TArg<Poly>, B: number): boolean => {
  ...
  for (let i = 0; i < N; i++) if (iabs(crystals.smod(p[i])) >= B) return true;
  return false;
};
...
function getDilithium(opts_: TArg<DilithiumOpts>): TRet<DSA> {
  ...
  const decompose = (r: number) => {
    ...
    const r1 = idiv(rPlus - r0, 2 * GAMMA2) | 0;
  ...
  const HINT_M = idiv(Q - 1, 2 * GAMMA2);
  ...
  const Power2Round = (r: number) => {
    ...
    return { r1: idiv(rPlus - r0, 2 ** D) | 0, r0 };
  };
