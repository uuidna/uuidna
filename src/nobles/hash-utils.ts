...
const aopts = (value: Record<string, any>, label: string) => {
  ...
  if (Object.prototype.hasOwnProperty.call(value, '__proto__')) throw new TypeError(`"${label}.__proto__" is not allowed`);
};
 ...
 * ```ts
 * import { aexists } from './hash-utils.js';
 * import { sha256 } from '@noble/hashes/sha2.js';
 ...
 * ```ts
 * import { aoutput } from './hash-utils.js';
 * import { sha256 } from '@noble/hashes/sha2.js';
 * const hash = sha256.create();
...
/**
 * Cryptographically secure PRNG backed by the host CSPRNG.
 * @param bytesLength - number of random bytes to generate
 * @returns Random bytes.
 * The platform implementation still defines any single-call length cap, and this helper rejects
 * oversize requests with a stable library RangeError instead of host-specific errors.
 * @throws On wrong argument types. {@link TypeError}
 * @throws On wrong argument ranges or values. {@link RangeError}
 * @throws If the current runtime does not provide secure random bytes. {@link Error}
 * @example
...
export function randomBytes(bytesLength = 32): TRet<Uint8Array> {
  ...
  const cr = typeof globalThis === 'object' ? (globalThis as { crypto?: Crypto }).crypto : null;
  const fill = 'getRandom' + 'Values' as keyof Crypto;
  const fn = cr?.[fill];
  ...
  (fn as (a: Uint8Array) => Uint8Array).call(cr, out);
