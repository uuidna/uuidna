 ...
 * @module
 */
import { imin } from './imath.js';
import { setU64FromNum } from './u64.js';
import { abytes, aexists, aoutput, createView, type Hash, type TArg, type TRet } from './hash-utils.js';
...
export abstract class HashMD<T extends HashMD<T>> implements Hash<T> {
  ...
  // across HashMD shapes makes it megamorphic: the tuple is materialized, cannot be explicitly
  // wiped, and clone-heavy KDFs regress.
  ...
  update(data: TArg<Uint8Array>): this {
    ...
    for (let pos = 0; pos < len;) {
      const take = imin(blockLen - this.pos, len - pos);
