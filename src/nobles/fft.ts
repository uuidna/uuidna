 ...
 * @module
 */
import { aarray, validateObject, type TArg } from './curve-utils.js';
import { iclz32, imin } from './imath.js';
import { validateField, type IField } from './modular.js';
 ...
 *
 * ```ts
 * import { rootsOfUnity } from './fft.js';
 ...
 *
 * ```ts
 * import { FFTCore, rootsOfUnity } from './fft.js';
 * import { Field } from './modular.js';
 * const Fp = Field(17n);
 ...
 *
 * ```ts
 * import { FFT, rootsOfUnity } from './fft.js';
 * import { Field } from './modular.js';
 * const Fp = Field(17n);
...
export type PolyFn<P extends PolyStorage<T>, T> = {
  ...
  lagrange: {
    ...
    evaluate: (a: P, x: T, brp?: boolean) => T;
  };
  /**
 ...
 *
 * ```ts
 * import { poly, rootsOfUnity } from './fft.js';
 * import { Field } from './modular.js';
 * const Fp = Field(17n);
...
export function poly<T, P extends PolyStorage<T>>(
  ...
  return {
    ...
    extend: (a: P, len: number): P => {
      ...
      for (let i = 0; i < imin(a.length, len); i++) out[i] = a[i];
      return out;
    },
    degree: (a: P): number => {
      checkLength(a);
    ...
    monomial: {
      ...
      evaluate: (a: P, x: T): T => {
        checkLength(a);
        // Same as evaluate(a, monomialBasis(x, a.length)), but it is faster this way
        let acc = F.ZERO;
