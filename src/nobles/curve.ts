...
import { iabs, iceilDiv, idiv, imax } from './imath.js';
 ...
 *
 * ```ts
 * import { probeRandomBytes } from './curve.js';
 * import { randomBytes } from './hash-utils.js';
...
function wnafWalk<P extends { double(): P; add(other: P): P; negate(): P }>(
  ...
  let max = 0;
  for (const d of digits) max = imax(max, d.length);
  let acc = zero;
  for (let bit = max - 1; bit >= 0; bit--) {
    ...
    for (let i = 0; i < digits.length; i++) {
      ...
      if (w) {
        const item = tables[i][(iabs(w) - 1) >> 1];
 ...
 *
 * ```ts
 * import { ScalarMultiplier } from './curve.js';
 * import { p256 } from '@noble/curves/nist.js';
...
export class ScalarMultiplier<PC extends PC_ANY> {
  ...
  private buildWnafTable(point: PC_P<PC>, W: number, bits: number): WnafPrecomputeEntry<PC_P<PC>> {
    ...
    const windows = iceilDiv(bits, W) + 1;
    const half = 2 ** (W - 1);
    const comp: PC_P<PC>[] = [];
    let base = point;
    for (let w = 0; w < windows; w++) {
  ...
  private wnafCachedCT(precomputes: WnafPrecomputeEntry<PC_P<PC>>, n: bigint): MulResult<PC_P<PC>> {
    ...
    for (let w = 0; w < windows; w++) {
      ...
      const idx = iabs(digit) - 1; // -1 for zero digits: matches nothing, `sel` unused
      let sel = comp[start];
  ...
  setWindowSize(point: PC_P<PC>, W: number): void {
    ...
    const windows = iceilDiv(this.bits + BLIND_BITS, W) + 1;
 ...
 *
 * ```ts
 * import { mulAddUnsafe } from './curve.js';
 * import { p256 } from '@noble/curves/nist.js';
...
export function pippenger<P extends CurvePoint<any, P>, PC extends CurvePointCons<P>>(
  ...
  const lastBits = idiv(fieldN.BITS - 1, windowSize) * windowSize;
  let sum = zero;
 ...
 *
 * ```ts
 * import { interleavedMSMUnsafe } from './curve.js';
 * import { p256 } from '@noble/curves/nist.js';
