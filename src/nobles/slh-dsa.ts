...
import { bytesToNumberBE, numberToBytesBE } from './curve-utils.js';
import { iceilDiv, idiv } from './imath.js';
import { sha256, sha512 } from './sha2.js';
import { shake256 } from './sha3.js';
import { concatBytes, createView, type CHash } from './hash-utils.js';
import {
  abytes,
  checkHash,
  cleanBytes,
...
function gen(opts: SphincsOpts, hashOpts_: TArg<InternalSphincsHashOpts>): TRet<SphincsSigner> {
  ...
  const WOTS_LOGW = 4;
  const WOTS_LEN1 = idiv(8 * N, WOTS_LOGW);
  const WOTS_LEN2 = N <= 8 ? 2 : N <= 136 ? 3 : 4;
  const TREE_HEIGHT = idiv(H, D);
  ...
  const setAddr = (
    ...
    const type = Object.prototype.hasOwnProperty.call(opts, 'type') ? opts.type : undefined;
    const height = Object.prototype.hasOwnProperty.call(opts, 'height') ? opts.height : undefined;
    const tree = Object.prototype.hasOwnProperty.call(opts, 'tree') ? opts.tree : undefined;
    const layer = Object.prototype.hasOwnProperty.call(opts, 'layer') ? opts.layer : undefined;
    const index = Object.prototype.hasOwnProperty.call(opts, 'index') ? opts.index : undefined;
    const chain = Object.prototype.hasOwnProperty.call(opts, 'chain') ? opts.chain : undefined;
    const hash = Object.prototype.hasOwnProperty.call(opts, 'hash') ? opts.hash : undefined;
    const keypair = Object.prototype.hasOwnProperty.call(opts, 'keypair') ? opts.keypair : undefined;
    const subtreeAddr = Object.prototype.hasOwnProperty.call(opts, 'subtreeAddr') ? opts.subtreeAddr : undefined;
    const keypairAddr = Object.prototype.hasOwnProperty.call(opts, 'keypairAddr') ? opts.keypairAddr : undefined;
  ...
  const hashMsgCoder = splitCoder(
    'hashedMessage',
    iceilDiv(A * K, 8),
    iceilDiv(TREE_BITS, 8),
    iceilDiv(TREE_HEIGHT, 8)
  );
  // `pkSeed` is the full public key byte string `PK.seed || PK.root`; after splitting `Hmsg`,
  // mask away any spare high bits so tree index and leaf index match the spec's final mod-2^k steps.
  const hashMessage = (
    R: TArg<Uint8Array>,
  ...
  // Iterative XMSS node/sign core: mutate treeAddr in place, collapse completed
  ...
  // Fuse XMSS sign with the subtree-root computation needed by hypertree sign, so one tree walk
  ...
  (pub_seed: TArg<Uint8Array>, sk_seed?: TArg<Uint8Array>): TRet<Context> => {
    ...
    */
    ...
    function mgf1(seed: TArg<Uint8Array>, length: number, hash: ShaType): TRet<Uint8Array> {
      stats.mgf1++;
      const out = new Uint8Array(iceilDiv(length, hash.outputLen) * hash.outputLen);
      // NOT 2^32-1
