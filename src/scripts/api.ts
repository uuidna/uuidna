...
// ── AND WHAT GIT WOULD COMMIT INCLUDES THE FILE NOT YET ADDED (2026-09-15). Deferring the untracked set made the
// local guard pass a new file that the landing then committed and the committed-tree certification refused — two
// landings in one day, each stopped after the whole heal. A finder that reads the tree reads what `git add` would
// carry: tracked, plus untracked-and-not-ignored. Only a gitignored file is outside the landing, so only it is
// deferred. inFlightFiles above keeps its meaning for land, which leaves a peer's unstaged files out of its stage.
const _committable = new Map<string, Set<string>>()   // keyed by root, like _inFlight
/** listCommittable(pathspec, root) → tracked ∪ untracked-not-ignored under the pathspec: the set a landing commits. */
export const listCommittable = (pathspec: readonly string[] = ['.'], root: string = ROOT): string[] => {
  const out = cpm().execFileSync('git', ['ls-files', '-z', '--cached', '--others', '--exclude-standard', '--', ...pathspec, ...TRACKED_EXCLUDE], {
    cwd: root, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024,
  })
  return [...new Set(out.split('\0').filter(Boolean))]
}
/** judged(files) → the files a finder may hold to account: every given file a landing would commit; the rest
 *  (gitignored, or outside any repository) is deferred by name. */
export const judged = (files: readonly string[], root: string = ROOT): { files: string[]; deferred: string[] } => {
  let keep = _committable.get(root)
  if (!keep) {
    try { keep = new Set(listCommittable(['.'], root)) } catch { keep = new Set(files) }
    _committable.set(root, keep)
  }
  const deferred = files.filter((f) => !keep.has(f))
  return { files: files.filter((f) => keep.has(f)), deferred }
}
...
export const DRAIN_PATHS: readonly string[] = [
  ...
  'docs/school.md',
  // the rights page is generated from the instrument table and the land-rights wing
  'docs/rights.md',
  ...
  'src/cloudflare-templates-data.ts',
  ...
  // the school's served courses — one lesson file per wing, rewritten whole by gen-school-lessons
  'docs/public/school',
  '.gitattributes',
  'packages', 'lean',
...
export const RECONCILE_OUTPUTS: Readonly<Record<string, readonly string[]>> = {
  ...
  'gen-llm': ['llm.txt'],
  'gen-cloudflare-templates': ['src/cloudflare-templates-data.ts'],
  ...
  'gen-rights-page': ['docs/rights.md'],
  'gen-school-lessons': ['docs/public/school'],
  ...
  'gen-receipts': ['lean/publications-receipt.json', 'lean/coverage-receipt.json', 'lean/decode-receipt.json'],
