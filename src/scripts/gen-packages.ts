#!/usr/bin/env node
// gen-packages — THE SPLIT IS COMPUTED
...
// owning package gains it on the next `npm run gen:packages` — the split tracks the surface.
...
if (VERIFY && drift) { console.error(`✗ gen-packages — ${drift} package surface(s) drift; the split is computed.`); process.exit(1) }
