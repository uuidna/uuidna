...
**The editorial desk writes what the ledger proves.** Every page in this section, including this index, is computed from the ledger — nothing here is authored by a person, and every claim carries its citation. One article per wing — the headline is the principle, the lede is the wing's own header, every claim cites its theorem, and the honest-scope box is part of the article because it is part of the proof. Regenerated by \`npm run editorial\` before any publication; verified by the same desk that wrote it.
