...
// fact in JS, GENERATE its `by decide` theorem, VERIFY sorry-free. Integrity.
...
const FACTS = [
  { key: 'two_coins',
    why: 'The two coins — the conserved fair-exchange invariant, 110 − 108 = 2. A measure of work saved (recompute − verify).',
    js: () => 110 - 108 === 2,
    lean: 'theorem two_coins : 110 - 108 = 2 := by decide' },

  { key: 'two_coins_is_double_torus',
    why: 'The two coins are the topology. The invariant is geometric.',
    js: () => 2 * 2 - 2 === 2,
  ...
  { key: 'captain_computes_only_with_two_coins',
    why: 'uuidna computes ONLY IF the captain coins are considered: the conserved save of 64 is reached IFF exactly two coins are put in — 32·c = 64 ⟺ c = 2, for every c. The two coins are necessary; with any other count the fold does not conserve its advantage (recompute − verify), so the computation is not admitted.',
  ...
  { key: 'superposition_outcomes_to_64',
    why: 'Direct possible outcomes: n qubits give 2ⁿ basis outcomes — [1,2,4,8,16,32,64] for n = 0..6, reaching 64 exactly at the 6-qubit / 64-bit scale. Exponential, counted.',
  ...
  { key: 'coin_is_one_qubit',
    why: 'One coin is one qubit: a two-state basis — 2¹ = 2 outcomes, the coin\'s two faces (|0⟩ and |1⟩). Classical two-state accounting in the state-vector simulator.',
    js: () => 2 ** 1 === 2,
    lean: 'theorem coin_is_one_qubit : (2:Nat)^1 = 2 := by decide' },

  { key: 'captain_coins_deliver_two_qubits_at_128_bits',
    why: 'The two captain coins DELIVER two qubits at a COST of 128 bits: coins() = 2 → two qubits spanning 2² = 4 basis states, carried by one 128-bit uuid = two 64-bit coins (128 = 2·64 = 2⁷). Two coins in, a 2-qubit address out, priced at 128 bits — the 64→128 fuse, counted. Classical accounting.',
  ...
  { key: 'captain_commission_two_per_110',
    why: 'The captain\'s commission is TWO on each 110 bits — 110 − 108 = 2 to the captain, 108 delivered net. It is the two coins read as a commission rate: passengers PAY the coins (the measured bill), crew MINT them (sealing diamonds), and the captain\'s cut is the conserved 2. A measured commission on work saved.',
  ...
  { key: 'commercial_saves_and_captain_earns',
    why: 'A commercial package SAVES significantly at scale AND the captain still earns: recompute 110 − verify 1 = 109 bits saved for the passenger, while the captain\'s commission stays the conserved 2 (110 − 108), and the saving DWARFS the commission (109 > 2). Coins are minted, the passenger saves, the captain earns — no one loses. The arithmetic of the measured advantage (recompute − verify).',
