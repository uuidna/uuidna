...
const leadsHeld = ((): number => {
  try {
    const j = JSON.parse(readFileSync(join(ROOT, 'lean', 'leads.json'), 'utf8')) as { held?: unknown[] }
    return Array.isArray(j.held) ? j.held.length : 0
  } catch { return 0 }
})()
...
const PRACTICES: string[] = [
  ...
  `8. **Discuss the open at school** — after a wave of external research, every unverified lands in the held
   record (${leadsHeld} this generation) and the [open questions](/open-questions) curriculum; local labs
   (\`labOf\`) recompute only the sealed half. Silence never refutes (${cite('silence_never_refutes')}).
   *Practice:* open [the leads](/leads), pick one door, name a finite structure, deposit the two coins
...
console.log(`✓ gen-school — ${PRACTICES.length} practices + ${byWing.size} wings + ${CLAY.length} Clay lessons generated; figures: ${fig.monographs} monographs + ${fig.auras} auras = ${fig.redundant} (${fig.hexbits} hexbits, ${fig.bits} bits)`)
