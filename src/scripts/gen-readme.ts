...
import { theorems, statementCensus, runTrial, coins, RESEARCH_SOURCE_NAMES, byGravity, decidedMass, gravityOf, wingRatings, ledgerMass, hexbitsOf, HANDLE_SPAN, COINS } from '../index.js'
...
function generateReadme(): string {
  const servedCeiling = Number((readFileSync(join(ROOT_DIR, 'src', 'mcp.ts'), 'utf8')
...
${byGravity().slice(0, 7).map((t, i) => `${i + 1}. **\`${t.key}\`** — ${decidedMass(t).toLocaleString('en-US')} superpositions, ${gravityOf(t)} hexbits for the two coins, in [${t.file}](lean/${t.file})\n   ${(t.name ?? '').split('.')[0]}.`).join('\n')}
