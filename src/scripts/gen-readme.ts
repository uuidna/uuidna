...
import { MAX_SERVED_QUBITS } from '../mcp.js'   // the served ceiling, IMPORTED — never scraped from source text
import { execSync } from 'child_process'
import { writeFileSync, readFileSync } from 'fs'
import { join } from 'path'
import { ROOT as ROOT_DIR } from './api.js'
import { theorems, statementCensus, runTrial, coins, RESEARCH_SOURCE_NAMES, byGravity, decidedMass, gravityOf, isUnbound, wingRatings, ledgerMass, hexbitsOf, HANDLE_SPAN, COINS } from '../index.js'
...
function generateReadme(): string {
  // IMPORTED, NOT REGEXED OUT OF SOURCE TEXT. This read `src/mcp.ts` as a string and matched
  // `/n > (\d+)\) throw new Error\('qubits must be an integer in 1\.\.\d+/` to recover the served ceiling —
  // publishing a number this file had scraped from another file's punctuation, under a header promising every
  // figure is "COMPUTED at generation from the ledger and the wired code". It also made the bound unnameable:
  // turning `n > 12` into a constant would have silently broken this match and printed 0 qubits served. The
  // constant is now exported and imported, so the README and the guard cannot disagree, and `axiom-hunt` can
  // finally see the bound at all.
  const servedCeiling = MAX_SERVED_QUBITS

const stats = getSystemStats()

  return `# ${stats.name} — ${stats.distinct} distinct theorems under ${stats.theorems} keys, ${stats.coins} coins, one receipt
...
${ledgerMass().toLocaleString('en-US')} superpositions across ${wingRatings().length} wings.

${byGravity().slice(0, 7).map((t, i) => `${i + 1}. **\`${t.key}\`** — ${decidedMass(t).toLocaleString('en-US')} superpositions, ${isUnbound(t) ? 'unbound (0 hexbits per dependency — nothing in the ledger pulls on it)' : `${gravityOf(t)} hexbits for the two coins`}, in [${t.file}](lean/${t.file})\n   ${(t.name ?? '').split('.')[0]}.`).join('\n')}
...
- **${MAX_MESSAGE_QUBITS} qubits** in the library register — ${2 ** MAX_MESSAGE_QUBITS} amplitudes held at once
- **${servedCeiling} qubits** served over MCP — ${2 ** servedCeiling} amplitudes, and the surface refuses more
- **${ledgerMass().toLocaleString('en-US')} superpositions** decided across the ledger, ${hexbitsOf(ledgerMass())} hexbits, every one settled \`by decide\`
- **${MAX_MESSAGE_QUBITS - servedCeiling} qubits** of reachability gap between the register and what is served
- **${theorems().filter((t) => t.file === 'Quantum.lean').length} quantum wing theorems**, **${theorems().filter((t) => t.file === 'Cipher.lean').length} cipher wing theorems**, **${theorems().length} ledger size**
...
A handle is eight hexbits, so it names **${HANDLE_SPAN.toLocaleString('en-US')}** superpositions — 16^8, every
