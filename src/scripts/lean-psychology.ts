...
const FACTS = [
  ...
  { key: 'big_five_factors_pentad',
    why: 'The Big Five (OCEAN) model is FIVE factors — a five-dimensional description, a pentad. This seals only the COUNT of the model\'s axes (five).',
    js: () => [0, 1, 2, 3, 4].length === 5,
    lean: 'theorem big_five_factors_pentad : (List.range 5).length = 5 := by decide' },

  { key: 'working_memory_span_seven',
    why: 'Miller\'s reported span — the "magical number seven, plus or minus two" — is the range [5, 9], width 4 (7−2 = 5, 7+2 = 9, 9−5 = 4). This seals the ARITHMETIC of the range Miller reported.',
    js: () => 7 - 2 === 5 && 7 + 2 === 9 && 9 - 5 === 4,
    lean: 'theorem working_memory_span_seven : (7 - 2 = 5) ∧ (7 + 2 = 9) ∧ (9 - 5 = 4) := by decide' },

  { key: 'hicks_law_three_bits',
    why: 'Hick\'s law relates decision time to the INFORMATION of a choice: n equally-likely options carry log₂ n bits, so 8 options are exactly 3 bits (2³ = 8). This seals the information-theoretic arithmetic's reaction time.',
    js: () => 2 ** 3 === 8,
    lean: 'theorem hicks_law_three_bits : 2^3 = 8 := by decide' },

  { key: 'signal_detection_two_by_two',
    why: 'A yes/no detection has a 2×2 = 4 outcome table — hit, miss, false alarm, correct rejection — the two truths crossed with the two responses. This seals the COUNTING of the outcome table.',
    js: () => 2 * 2 === 4,
    lean: 'theorem signal_detection_two_by_two : 2 * 2 = 4 := by decide' },

  { key: 'weber_fechner_log_ladder',
    why: 'Weber–Fechner: perceived intensity grows with the LOGARITHM of the stimulus, so a geometric stimulus ladder 1,2,4,8 (the powers 2⁰..2³) maps to equal perceived steps 0,1,2,3. This seals the log-ladder arithmetic.',
  ...
  { key: 'developmental_stages_double',
    why: 'Two named developmental models carry stage-counts in a doubling relation: Piaget names 4 stages and Erikson names 8, and 8 = 2·4. This seals only the ARITHMETIC of the reported counts (a named model has that many stages).',
    js: () => 2 * 4 === 8,
    lean: 'theorem developmental_stages_double : 2 * 4 = 8 := by decide' },

  { key: 'dunbar_layers_triple_and_round',
    why: 'Dunbar\'s social layers scale by about ×3 — 5, 15, then the exact 45 and 135 — but are REPORTED rounded to 50 and 150 (the same rounding gap the photography stops carry: 50−45 = 5, 150−135 = 15). This seals the ratio arithmetic and the rounding.',
...
emit({
  ...
  header: 'THE INSTRUMENT — the ARITHMETIC of psychology\'s instruments and named models (the Likert midpoint, the Big Five count, Miller\'s span, Hick\'s bits, the detection table, the Weber–Fechner ladder, Dunbar\'s rounded layers) and ONLY that.',
