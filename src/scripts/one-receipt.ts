...
export function drainGaps(): Gap[] {
  ...
  // ONE WRITER PER CONCRETE DRAIN FILE. RECONCILE_OUTPUTS is generator→outputs; the lead that cost a stale
  // captain-claims-complete.json (1307 theorems while the ledger passed 2000) was the inverse: a drain path
  // with ZERO owners, or two. Directory/glob paths are umbrellas and are not counted — only files with an
  // extension and no wildcard. Same script named on both reconcile and docs:build is ONE owner.
  ...
  for (const [g, outs] of Object.entries(RECONCILE_OUTPUTS))
    for (const o of outs as readonly string[]) claim(o, g)
  for (const [g, outs] of Object.entries(DOCS_BUILD_OUTPUTS))
    for (const o of outs as readonly string[]) claim(o, g)
  ...
  for (const p of DRAIN_PATHS) {
    ...
    if (who.length === 0)
      gaps.push({
        what: `${p} is a drain path with ZERO declared writers — regenerated by nobody, staged by the drain`,
        fix: `name its single writer in RECONCILE_OUTPUTS / DOCS_BUILD_OUTPUTS, or in DRAIN_WRITERS (src/scripts/api.ts) if it rides outside the manifest (lean / cron / session)`,
      })
    else if (who.length > 1)
      gaps.push({
        what: `${p} has ${who.length} declared writers (${who.join(', ')}) — two owners is drift waiting to happen`,
        fix: `keep exactly one writer in the declarations; if two chains run the same script, they already share one name`,
      })
  }
  return gaps
}
...
export function mirrorGaps(): Gap[] {
  const gaps: Gap[] = []
  const SAFE = Number.MAX_SAFE_INTEGER // 2^53 - 1 — the platform's own constant, which the comment already named
...
export function fold() {
  ...
  const d = a.ten
  if (!dec || dec.ray !== d.ray || dec.hue !== d.hue || dec.residue !== d.residue || dec.sat !== d.sat || dec.light !== d.light) {
  ...
  const aura = { rgb: a.rgb, hsl: a.hsl, alphabet, movie, lessons, report, session, coverage: Object.fromEntries(REPORT), dimensions: { residue: d.residue, ray: d.ray, wave: d.wave, hue: d.hue, sat: d.sat, light: d.light, period: d.period, rotation: d.rotation, glow_inner: d.glowInner, glow_outer: d.glowOuter }, free: [...FREE_KEYS], hz: A432_HZ, note: 'the colour is a reversible harmonic message — ten dimensions, seven compactified; decoration made readable, still not physics' }
...
export function citationsGaps(): Gap[] {
  ...
  const shipped = files.filter((f) => !f.includes('/tests/') && !isTestSource(f))
  ...
  for (const key of removed) {
    ...
    const where = files.filter((f) => !f.includes('/tests/') && !isTestSource(f)).filter((f) => { try { return fileText(join(ROOT, f)).includes(key) } catch { return false } })
    gaps.push({
...
/** constantGaps() — A DERIVATION WRITTEN IN PROSE IS A DERIVATION THE COMPILER CANNOT CHECK.
 *
 ...
 *
 *  WHY THIS SHAPE AND NOT "EVERY LITERAL THAT APPEARS IN A THEOREM". That finder was built first and thrown
 *  away: 518 sealed integers matched 2840 literals across 132 files, and almost all were coincidence —
 *  `desc.length < 50`, `slice(0, 200)`, `50: '#f0f9ff'`. `100` is "sealed" in 48 theorems because it is a
 *  percentage. A value carries no evidence that two uses are the same QUANTITY. The comment does: when a line
 *  writes the arithmetic itself, the author has already told us the number is derived. 2840 unreadable hits
 *  became 4 exact ones by asking the better question.
 *
 *  GENERATORS ARE EXEMPT BY PATH, the way os/ and drivers/ are the named non-determinism boundary: scripts/lean-*
 *  and scripts/gen-* EMIT theorem statements, so they must carry those statements' numbers literally. */
export function constantGaps(): Gap[] {
  const gaps: Gap[] = []
  // integer arithmetic written in prose: 9·7·4, 360/9, 504 − 72, 2^53 - 1. Exact integers only — a comment
  // whose arithmetic is fractional is describing something else, and this finder does not guess.
  const norm = (e: string): string => e.replace(/[·×]/g, '*').replace(/[−–—]/g, '-').replace(/\s+/g, '')
  const evalInt = (e: string): number | null => {
    if (!/^\d+([*/+\-^]\d+)+$/.test(e)) return null
    let x = e
    for (let guard = 0; guard < 8 && /\^/.test(x); guard++) {
      x = x.replace(/(\d+)\^(\d+)/, (_m, a: string, b: string) => {
        let v = 1; for (let i = 0; i < Number(b); i++) v = v * Number(a); return String(v)
      })
    }
    if (!/^\d+([*/+\-]\d+)*$/.test(x)) return null
    // left-to-right with precedence, integer-exact; anything fractional disqualifies the match
    const terms = x.split(/(?=[+\-])/)
    let total = 0
    for (const t of terms) {
      ...
      for (let i = 1; i < parts.length; i += 2) {
        op = parts[i]!; const n = Number(parts[i + 1])
        if (op === '*') cur = cur * n
        else { if (n === 0 || cur % n !== 0) return null; cur = cur / n }
      }
      total += sign * cur
    }
  ...
  for (const f of trackedFiles().filter((x) => x.startsWith('src/') && x.endsWith('.ts'))) {
    ...
    src.split('\n').forEach((raw, i) => {
      // A COMMENT LINE IS NOT AN ASSIGNMENT. Documentation that illustrates the very pattern this finder hunts
      // would otherwise be reported as the fault it describes — which happened on the first run: it named the
      ...
      const m = raw.match(/=\s*(\d{2,})\s*(?![\d\s]*[*/+\-^]\s*\w)[^/]*\/\/(.*)$/)
      if (!m) return
      const lit = Number(m[1])
      for (const e of m[2]!.matchAll(EXPR)) {
        if (evalInt(norm(e[1]!)) !== lit) continue
        gaps.push({
          what: `${f}:${i + 1} assigns the literal ${lit} while its own comment states the arithmetic that produces it (\`${e[1]!.trim()}\`) — the derivation is prose the compiler never checks`,
          fix: `compute it: write the expression the comment already states, so the value and its reason cannot drift apart (hexbit/index.ts is the pattern — the unit computes what it names)`,
        })
        return
      }
    })
