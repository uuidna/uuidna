...
// content) into src/lean/payload-sync.json, ready for a vanilla Payload instance to upsert-by-slug and
...
console.log(`✓ payload-sync — ${docs.length} docs (${parents} parents, ${docs.length - parents} nested children; ${published} published, ${docs.length - published} draft) → src/lean/payload-sync.json`)
