...
// A DOI ends where the sentence's punctuation begins: `…BF00196791.` and `…BF00196791` are one
// source, and matching the full stop made them two claimants for one paper. Trailing . , ; : are
// trimmed by the class rather than in the reader, so the change is inside the line ruleDigest()
// hashes — a rule that moved says so.
const WITNESS = /\b(NIST|CODATA|WGS ?84|IUPAC|SI\b|Gutenberg|Landauer|Eratosthenes|Fujishima|McCarty|Heidrich|Rossi|Runciman|Rathbun|Mathot|Day,|Wellman|ISO ?\d|RFC ?\d|physics\.nist\.gov|bomb calorimetry)\b|10\.\d{4,9}\/[^\s)'"]*[^\s)'".,;:]/
