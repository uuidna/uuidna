#!/usr/bin/env node
// gen-changelog-section — THE CALENDAR WRITES FACTS, NEVER MEANING. The quantum calendar ticks the odometer on its
...
// missing, so a human adding the story later is completing the entry, not correcting it.
//
...
// offenders is a MAP (address → the axioms it borrows), not a list — `.length` on it is undefined, which is how
...
- odometer: ${prev ?? '?'} → **${VERSION}** (single-digit, monotone, never returning)
