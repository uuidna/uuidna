...
const FACTS = [
  ...
  { key: 'each_key_bit_doubles',
    why: 'Each key bit doubles the space a forger must search: 2^11 = 2 · 2^10 (2048 = 2 · 1024). The cost of guessing a key is the key entropy — a bound set by the length.',
    js: () => 2 ** 11 === 2 * 2 ** 10,
  ...
  // ── THE BIRTHDAY BOUND TURNED ON THIS LEDGER'S OWN STRUCTURE. Computed from the ledger itself
  ...
  // about two — so the site is already updated when live checks it. That margin is a BUDGET
