...
const GENERATORS: Gen[] = [
  ...
  { file: 'gen-references.js', args: [], note: 'the external bibliography — DOIs scanned from the wings’ own signed prose and resolved from Crossref/DataCite. A NAMED NETWORK READ, cached to lean/references.json and src/references-resolved.ts so every downstream surface stays offline; wired here because a gen-* that writes and is never run leaves its file to rot while git reports the tree clean' },
  ...
  { file: 'gen-school-lessons.js', args: [], note: 'docs/public/school — one served lesson file per course and the catalogue, each lesson composed from its ledger row with its instantly checked exercise where the evaluator decides one' },
  ...
  { file: 'gen-bound-census.js', args: [], note: 'the bound census sealed to lean/bound-census.json — every List.range widened by one and re-decided, incrementally by statement (an unchanged ledger costs one digest)' },
  { file: 'gen-missions.js', args: [], note: 'the mission board — docs/missions.md and the baked slice uuidna_missions serves at the edge, derived from the rosetta mirror, the bound census and the findings' },
  ...
  { file: 'gen-rights-page.js', args: [], note: 'docs/rights.md — the public rights door: the instrument table and the land-rights wing rendered, nothing written per instrument' },
