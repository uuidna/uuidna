...
test('deployPathRecomputeGaps fires on crafted next/land/deploy-run', () => {
  ...
  assert.ok(shipZoneSoft("node dist/scripts/cloudflare-zone.js --soft"))
  assert.equal(shipZoneSoft("node dist/scripts/cloudflare-zone.js"), false)
  assert.ok(zoneFailsOpen("const needScope = true\nconst failed = 1\nif (failed) console.log('complete')"))
  assert.equal(zoneFailsOpen("const soft = process.argv.includes('--soft')\nconst needScope = true\nconst broken = failed > 0 || needScope\nif (broken && !DRY && !soft) { process.exitCode = 1 }"), false)
  assert.ok(landDepositSoft("console.log(run('node dist/scripts/receipt-deposit.js HEAD').out)"))
  assert.equal(landDepositSoft("const deposited = run('node dist/scripts/receipt-deposit.js HEAD')\nif (!deposited.ok) process.exit(1)"), false)
  assert.ok(landForgeSoft("run('node dist/scripts/post-push.js HEAD --wait')\nconsole.log('done')"))
  assert.equal(landForgeSoft("const forge = run('node dist/scripts/post-push.js HEAD --wait')\nif (!forge.ok) process.exit(1)"), false)
  assert.ok(skipHooksHot("git push --no-verify origin main"))
  assert.equal(skipHooksHot("git push origin main"), false)
