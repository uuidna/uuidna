...
let fillGapsFail: string | null = null
let fillGapsAutoReceipt: string | null = null

console.log(`\n  next — the 777 self-trial · feeding uuidna ${VERSION} to its own trials${FAST ? '  (hexbit-fast · O(1) from the seal)' : FULL ? '  (full audit + trial)' : ''}${PUSH ? ' · push' : ''}\n`)
console.log(FULL
...
if (!NO_AUTO) {
  try {
    ...
    if (hasDeskAutomatableWork(survey)) {
      ...
      fillGapsAutoReceipt = mergeFillGapsReceipts(arc.receipt, advSnap.receipt)
      if (!arc.ok) {
        fillGapsFail = `fill-gaps: stopped at ${arc.failedPhase} during the auto arc before the trial`
        console.error(`\n  ✗ next · fill-gaps stopped at ${arc.failedPhase}; partial receipt ${fillGapsAutoReceipt}\n`)
      } else {
        console.log(`\n  ✓ next · fill-gaps — ${arc.plan.length} phase(s); merged receipt ${fillGapsAutoReceipt}\n`)
      }
    } else {
  ...
  if (PUSH) {
    ...
    try {
      execSync(`git push origin ${branch}`, {
        cwd: ROOT,
        stdio: 'inherit',
        env: { ...process.env, UUIDNA_NEXT_FUSED: readiness },
      })
      console.log(`\n  ✓ next · push — origin/${branch} published; readiness receipt ${readiness}; fused gate ${elapsed}s\n`)
    } catch {
      fails.push('push: git push failed after readiness')
      ready = false
      console.error('\n  ✗ next · push — git push failed after readiness\n')
    }
  } else {
...
try {
  ...
  if (end.harvest > 0) leverage.push({
    ...
    closes: `${end.harvest} TRUE-unsealed fragment(s) still waiting — desk proposes, kernel disposes; never rewrite usable_gap_is_two_to_eighty`,
  })
  const desk = end.automatable.filter((b) => b.kind !== 'guard-heal')
  if (desk.length && !fillGapsAutoReceipt) leverage.unshift({
  ...
  else if (desk.length && fillGapsAutoReceipt) leverage.unshift({
    act: 'npm run x -- fill-gaps  # optional re-run — derive-surfaces can surface new open-leads',
    closes: `${desk.map((b) => `${b.kind} (${b.count})`).join(', ')} — auto arc already ran (receipt ${fillGapsAutoReceipt})`,
  })
...
process.exitCode = ready && fails.length === 0 ? 0 : 1
