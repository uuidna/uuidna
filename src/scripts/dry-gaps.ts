...
export function dryGaps(): { gaps: Gap[]; scripts: number } {
  ...
  const door = (name: string, body: string): void => {
    if (!/quantum\/os\/cli\.js/.test(body))
      gaps.push({ what: `${name} does not enter uuidnaOS (cli.js) — a script beside the hex image is a bypass`, fix: `edit ${name}: node dist/quantum/os/cli/index.js only; missing dist is exit 1` })
    if (bypass(body))
