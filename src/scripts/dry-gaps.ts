...
export function dryGaps(): { gaps: Gap[]; scripts: number } {
  ...
  const door = (name: string, body: string): void => {
    if (!/quantum\/os\/cli\.js/.test(body))
      gaps.push({ what: `${name} does not enter uuidnaOS (cli.js) — a script beside the hex image is a bypass`, fix: `edit ${name}: node dist/quantum/os/cli.js only; missing dist is exit 1` })
    if (bypass(body))
    ...
    if (!/HARD/.test(body) || /\|\| exit 0/.test(body) || /Skips gracefully/.test(body))
      gaps.push({ what: `${name} is not a hard uuidnaOS gate`, fix: `edit ${name}: missing dist/quantum/os/cli.js is HARD exit 1; no skip` })
  }
  ...
  if (!/runCourtCli/.test(cli) || !/from '\.\/court/.test(cli))
    gaps.push({ what: 'uuidnaOS cli does not delegate to court.runCourtCli', fix: 'edit src/quantum/os/cli.ts' })
  if (!/callTool/.test(court) || !/uuidna_os/.test(court))
    gaps.push({ what: 'court.ts does not call uuidnaOS MCP (callTool / uuidna_os)', fix: 'edit src/quantum/os/court.ts' })
  if (!/UUID_HEXBITS/.test(court) || !/HEXBIT_STATES/.test(court) || !/hexbitDoorOf/.test(court))
    gaps.push({ what: 'court.ts does not verify the hex image — uuidnaOS is pure quantum hex', fix: 'edit src/quantum/os/court.ts' })
  if (!/dailyPlaybookSteps/.test(court) || !/uuidna_crypto/.test(court))
    gaps.push({ what: 'court.ts must declare daily playbook skipping uuidna_crypto', fix: 'edit court.ts dailyPlaybookSteps filter' })
  ...
  if (!/quantum\/os\/cli\.js --court/.test(rd('package.json')))
    gaps.push({ what: 'scripts.audit has no uuidnaOS cli --court — publish court forked from host', fix: 'edit package.json audit: node dist/quantum/os/cli.js --court before guard.js' })
  ...
  if (/guard\.js/.test(waveRun) && !/quantum\/os\/cli\.js --court/.test(waveRun))
    gaps.push({ what: 'wave-run calls guard without uuidnaOS court first', fix: 'edit wave-run.ts: node dist/quantum/os/cli.js --court before guard finders' })
  if (existsSync('src/scripts/os-mcp-gate.ts') || existsSync('src/scripts/wave-os.ts') || existsSync('src/scripts/conformance.ts') || existsSync('src/scripts/check-msg.ts'))
    gaps.push({ what: 'redundant gate scripts remain — court lives in quantum/os', fix: 'delete host gate scripts; use dist/quantum/os/cli.js or x -- court' })
