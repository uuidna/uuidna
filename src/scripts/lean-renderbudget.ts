...
const FACTS = [
  ...
  { key: 'the_cube_filled_and_the_tesseract_opens', name: '16³ = 4096 keys are behind the ledger; the form being filled is 16⁴ = 65536, sixteen cubes', skill: 'render-budget',
    why: 'A TEST THAT SAID "THE CUBE IS NOT YET FULL" FAILED ON A FULLER LEDGER. The sealed geometry counts every theorem as a vertex and the gap to the next power of the hexbit as the uncomputed; the next power was written as the cube, 16³, and the ledger passed 4096 keys, so the gap went negative and a true statement about growth read as a failure. The form climbs: the cube is the one already filled, the tesseract 16⁴ = 65536 is the one being filled, and it is exactly sixteen cubes. The live count stays out of the statement on purpose — a theorem that froze it would be false by the next landing.',
    js: () => 16 * 16 * 16 === 4096 && 4096 * 16 === 65536 && 65536 / 4096 === 16 && 4096 < 65536,
    lean: 'theorem the_cube_filled_and_the_tesseract_opens : (16 ^ 3 = 4096) ∧ (16 ^ 4 = 65536) ∧ (65536 / 4096 = 16) ∧ (4096 < 65536) := by decide' },
]
