...
import { plan, kindOf, runPlan, pool, label, type Instrument } from './gate-all.js'

const CHAIN = 'npm run lean && npm run build && node dist/scripts/gen-lines.js && node dist/scripts/guard.js'
  + ' && node dist/scripts/gen-reports.js && node dist/scripts/spin.js && node dist/quantum/os/cli/index.js --court'
...
test('every failing CHECK is reported in a single pass', async () => {
  const failing = new Set(['node dist/scripts/guard.js', 'node dist/scripts/spin.js', 'node dist/quantum/os/cli/index.js --court'])
...
test('a check failing never stops the generators or the checks that follow it', async () => {
  ...
  assert.equal(verdicts.length, 9)
  assert.equal(verdicts.filter((v: { exit: number }) => v.exit !== 0).length, 1)
})
...
test('an all-green chain reports no failures and never aborts', async () => {
  ...
  assert.equal(aborted, null)
  assert.equal(verdicts.filter((v: { exit: number }) => v.exit !== 0).length, 0)
})
...
test('every instrument names the arms it voids, and the invisible one is covered', () => {
  ...
  const covers = (cmd: string) => INSTRUMENTS.filter((i: Instrument) => i.covers.some((re: RegExp) => re.test(cmd))).map((i: Instrument) => i.file)
  ...
  const lean = INSTRUMENTS.find((i: Instrument) => i.file === 'lean')!
...
test('labels stay short enough to read as a table', () => {
  ...
  assert.equal(label("node --test 'dist/**/*.test.js'"), 'test dist', 'a test step names its suite — the 50-second stranger of lead 132b, never again bare "node"')
