...
//
// So the SSG runs where the memory is — a prior sealed `npm run docs:build` on the operator machine. Deploy
// (deploy-run.ts) VERIFIES that dist, then uploads with UUIDNA_SITE_BUILT=1. That is also the only path that
// keeps the deploy's laws: contribute-first deposit, origin-only tree, derived double proof. A git-connected
// container build keeps none of them, so a refusal here is the right answer twice over.
...
const isMain = process.argv[1]?.endsWith('ship-build.js') ?? false
if (isMain) {
  ...
  if (!site.present || !site.index) {
    ...
    console.error('')
    console.error('  IF YOU ARE THE OPERATOR: npm run docs:build (prior sealed SSG), then npm run ship.')
    console.error('  deploy-run verifies the dist, uploads with it in place, and keeps the contribute-first,')
