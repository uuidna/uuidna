...
{
  const themeDir = join(ROOT, 'docs', '.vitepress', 'theme')
  const panels = readdirSync(themeDir).filter((f: string) => f.endsWith('.vue')).sort()
  const pages = readdirSync(join(ROOT, 'docs')).filter((f: string) => f.endsWith('.md')).sort()
  // LIVE means mounted inside <ClientOnly> somewhere in docs — those compute on the reader's machine. Scanning
  // the built pages rather than trusting a list keeps this honest when a panel is added or unmounted.
  const md = pages.map((f: string) => readFileSync(join(ROOT, 'docs', f), 'utf8')).join('\n')
  const live = panels.filter((p: string) => {
    const tag = p.replace('.vue', '')
    const re = new RegExp('<ClientOnly>[\\s\\S]*?<' + tag + '\\b[\\s\\S]*?</ClientOnly>')
    return re.test(md)
  })
  const body = [
    '// GENERATED by scripts/gen-os — the monitor\'s panel and page inventory. Do not edit.',
    '// uuidnaOS asks the monitor what it can draw; this is the answer, enumerated at build and carried in the',
    '// bundle so a tab can answer it too.',
    'import type { MonitorInventory } from \'../index.js\'',
    '',
    'export const MONITOR_INVENTORY: MonitorInventory = {',
