...
// classical propositional logic as decidable truth tables — NOT a theorem prover, and NOT the predicate
// logic whose quantifiers range over unbounded domains. COMPUTE → GENERATE → VERIFY. Integrity.
...
const FACTS = [
  ...
  { key: 'manipulation_never_faster',
    ...
    js: () => Array.from({length: 16}, (_, i) => i + 1).every((n) => n < 2 ** n) && 16 < 2 ** 16,
    lean: 'theorem manipulation_never_faster : ((List.range\' 1 16).all (fun n => n < 2^n)) \u2227 (16 < 2^16) \u2227 (110 - 110 = 0) := by decide' },
  ...
  // decidable is the SHAPE of a claim (does evidence do any work on it?)
  ...
  { key: 'unfalsifiable_excludes_nothing',
    why: 'A CLAIM THAT FORBIDS NOTHING SAYS NOTHING. Information is what a claim EXCLUDES: over the four (e,c) rows the always-true claim rules out 0 of them, while the falsifiable e → c rules out exactly 1. Zero exclusions is zero content — measured.',
