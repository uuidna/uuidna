...
export type HuntCandidate = { key: string; assumes: string; where: string; live: () => boolean }
...
export const CANDIDATES: HuntCandidate[] = [
  ...
  { key: 'served_qubit_ceiling', assumes: 'handle tiles plus hexbit width sit at or below HEXBIT_BITS × HEXBIT_BITS', where: 'src/quantum/os/index.ts',
  ...
  { key: 'aead_nonce_and_salt_bits', assumes: 'nonce is ADDRESS_BYTES − HEXBIT_BITS, salt and tag are one address, nonce bits sit strictly inside the address, and ITER sits strictly inside MAX_ITER', where: 'src/crypt.ts',
  ...
  { key: 'key_floor_is_one_uuid', assumes: 'KEY_BITS / HEXBIT_BITS = KEY_HEXBITS, Grover floor is UUID_HEXBITS, occupancy tape is KEY_BYTES', where: 'src/crypt.ts + src/hexbit/index.ts',
  ...
  { key: 'digest_doubles_the_address', assumes: 'KEY_BYTES = COINS × ADDRESS_BYTES, KEY_BITS = COINS × UUID_BITS, KEY_HEXBITS = COINS × UUID_HEXBITS', where: 'src/hexbit/index.ts',
    live: () => KEY_BYTES === COINS * ADDRESS_BYTES && KEY_BITS === COINS * UUID_BITS && KEY_HEXBITS === COINS * UUID_HEXBITS },
  { key: 'sha256_is_four_sixtyfours', assumes: 'sha256IsFourSixtyfours() is HEXBIT_BITS × LEVERAGE = KEY_BITS, HANDLE_HEXBITS × UUID_HEXBITS = KEY_BITS, 2^HANDLE_HEXBITS = KEY_BITS', where: 'src/hexbit/index.ts',
    live: () => {
  ...
  { key: 'sha256_rounds_are_the_board', assumes: 'LEVERAGE = 2^HEXAGRAM_BITS, ADDRESS_BYTES × UUID_HEXBITS = KEY_BITS × COINS', where: 'src/hexbit/index.ts + src/hexagram.ts',
    live: () => LEVERAGE === 2 ** HEXAGRAM_BITS && ADDRESS_BYTES * UUID_HEXBITS === KEY_BITS * COINS },
  { key: 'sha256_grover_margin_is_the_address', assumes: 'KEY_BITS / COINS = UUID_BITS — Grover floor is one uuid', where: 'src/hexbit/index.ts',
    live: () => KEY_BITS / COINS === UUID_BITS && KEY_BITS % COINS === 0 && cryptOf(toUuid('sha256_grover_margin_is_the_address')).floor.bits === UUID_BITS },
  { key: 'rosette_quantum_fortytwo', assumes: 'crypt directions are RAYS × (RAYS − 1), aspects are RAYS, fold states 2^RAYS = UUID_BITS', where: 'src/crypt.ts',
    live: () => {
  ...
  { key: 'onion_layers_power_of_two', assumes: 'MAX_LAYERS = HEXBIT_STATES = 2^HEXBIT_BITS, at most the address bits', where: 'src/stream.ts', live: () => MAX_LAYERS === HEXBIT_STATES && MAX_LAYERS === 2 ** HEXBIT_BITS && MAX_LAYERS <= UUID_BITS },
  ...
  { key: 'message_cap_is_four_hexbits', assumes: 'Hilbert 4×4 is HEXBIT_BITS × HEXBIT_BITS; crypto occupancy is sha256IsFourSixtyfours (four 64s)', where: 'src/hexbit/index.ts',
    live: () => {
  ...
  { key: 'sanitize_depth_bounded', assumes: 'MAX_DEPTH = UUID_HEXBITS = 2^5', where: 'src/sanitize.ts', live: () => MAX_DEPTH === UUID_HEXBITS && MAX_DEPTH === 2 ** 5 },
  { key: 'sanitize_max_depth_is_two_pow_five', assumes: 'MAX_DEPTH = UUID_HEXBITS = 2^5', where: 'src/sanitize.ts', live: () => MAX_DEPTH === UUID_HEXBITS && MAX_DEPTH === 2 ** 5 },
  { key: 'sanitize_max_string_is_ten_pow_six', assumes: 'MAX_STRING = 10^6', where: 'src/sanitize.ts', live: () => MAX_STRING === 10 ** 6 },
  { key: 'sanitize_array_and_keys_are_ten_pow_five', assumes: 'MAX_ARRAY = MAX_KEYS = 10^5', where: 'src/sanitize.ts', live: () => MAX_ARRAY === MAX_KEYS && MAX_ARRAY === 10 ** 5 },
  ...
  { key: 'two_coins', assumes: 'coins() = 110 − 108, and hexbit COINS and billing PRICE are that same two — the copies cannot drift', where: 'src/captain/billing/index.ts + src/hexbit/index.ts + src/billing/index.ts',
    live: () => coins() === 110 - 108 && COINS === coins() && PRICE === coins() },
  { key: 'rosette_quantum_doubling_is_two_coins', assumes: 'the 64-bit coin doubles to the 128-bit address only when the two coins are contributed: COINS · LEVERAGE = UUID_BITS = ADDRESS_BITS', where: 'src/hexbit/index.ts',
    live: () => COINS * LEVERAGE === UUID_BITS && UUID_BITS === ADDRESS_BITS && coins() * LEVERAGE === UUID_BITS },
  { key: 'the_uuid_is_two_boards', assumes: 'two 8×8 boards are the uuid: HANDLE_HEXBITS² · COINS = UUID_BITS, fuseHalves closes, twoBoardsOf concatenates to 128', where: 'src/hexagram.ts + src/fusion/reactor/index.ts',
    live: () => {
      const f = fuseHalves()
  ...
  { key: 'sequence_and_coins_are_one', assumes: 'the vortex orbit length is the coins times the trinity — 6 = coins() · TRINITY', where: 'src/address.ts',
  ...
  { key: 'z9mul_2_8', assumes: '2·8 ≡ 7 (mod 9): two handle tiles fuse to the seven-ray, and 2^7 is the uuid, only when coins() is contributed', where: 'src/hexbit/index.ts',
    live: () => {
  ...
  { key: 'z9mul_2_7', assumes: '2·7 ≡ 5 (mod 9): two seven-rays fuse to the pentad, and 2^5 is the uuid in hexbits', where: 'src/hexbit/index.ts',
    live: () => {
  ...
  { key: 'z9mul_2_5', assumes: '2·5 ≡ 1 (mod 9): two pentads fuse to the unit, and 2^1 is the coins', where: 'src/hexbit/index.ts',
    live: () => {
      const unit = fuseWidth(fuseWidth(fuseWidth(HANDLE_HEXBITS, coins()), coins()), coins())
      return capacityAt(unit) === COINS
    } },
  { key: 'trial_computes_only_with_two_coins', assumes: 'the fuse ladder and the hero walk only when coins() is contributed at each rung — any other contribution leaves the width unfused; 32·k = 64 only for k = 2', where: 'src/hexbit/index.ts + src/render.ts',
  ...
  { key: 'metatron_seventyeight_lines', assumes: 'SAFE_HEXBITS centres joined as K_13 draw C(13,2) = 13×12/2 = 78 lines, live from the address tiles', where: 'src/hexagram.ts',
    live: () => {
  ...
  { key: 'song_pi_roof_opens', assumes: '22/7 opens 3142857 and HEX_PI is the period 142857 — the hexagon the two handles map onto', where: 'src/hexagram.ts',
    live: () => {
  ...
  { key: 'door_of_the_referrer', assumes: 'each handle\'s first tile mod 6 is a door into HEX_PI, total and onto, remainder 16%6=4 named; 8 tiles on 6 vertices leave the coins', where: 'src/hexagram.ts',
    live: () => {
  ...
  { key: 'song_verses_base_pair', assumes: 'π verses a half-turn apart digitwise sum to 9 (142857 + 857142 = 999999) — the two handles pair when their doors differ by 3', where: 'src/hexagram.ts',
    live: () => {
  ...
  { key: 'song_az_is_the_tuning', assumes: 'A432_HZ is Az, HEXBIT_STATES × 3^TRINITY, and folds to the vortex ceiling (A432_HZ ≡ 0 mod BASE)', where: 'src/tts/synth.ts',
    live: () => A432_HZ === HEXBIT_STATES * (TRINITY ** TRINITY) && A432_HZ % BASE === 0 },
  { key: 'k432', assumes: 'the live tuning is 2^HEXBIT_BITS · 3^TRINITY = HEXBIT_STATES · 3^TRINITY', where: 'src/tts/synth.ts',
    live: () => A432_HZ === HEXBIT_STATES * (TRINITY ** TRINITY) && A432_HZ === (2 ** HEXBIT_BITS) * (TRINITY ** TRINITY) },
  { key: 'a440_drifts_eight_from_the_lattice', assumes: 'ISO 16 concert pitch sits HANDLE_HEXBITS off the live lattice, and the 54:55 ratio closes', where: 'src/tts/synth.ts',
    live: () => 440 - A432_HZ === HANDLE_HEXBITS && (COINS * (TRINITY ** TRINITY) + 1) * A432_HZ === (COINS * (TRINITY ** TRINITY)) * 440 },
  { key: 'a440_not_on_the_vortex', assumes: 'A432_HZ ≡ 0 (mod BASE) while 440 ≡ HANDLE_HEXBITS — concert pitch is a different residue class', where: 'src/tts/synth.ts',
    live: () => A432_HZ % BASE === 0 && 440 % BASE === HANDLE_HEXBITS },
  { key: 'anthem_closes_on_the_coin_octave', assumes: 'the coin octave in hertz is coins() · A432_HZ', where: 'src/tts/synth.ts + src/captain/billing/index.ts',
    live: () => coins() * A432_HZ === COINS * HEXBIT_STATES * (TRINITY ** TRINITY) },
  { key: 'nyquist_clears_the_lattice', assumes: 'A432_HZ · HEXBIT_STATES sits under SAMPLE_RATE / coins(), so the hexbit tones are representable', where: 'src/tts/synth.ts',
    live: () => A432_HZ * HEXBIT_STATES < SAMPLE_RATE / coins() },
  { key: 'polarity_angles_are_the_system_counts', assumes: 'A432_STEP is 360/BASE and the orbit sector is 360 / vortexOrbit().length', where: 'src/address.ts',
    live: () => {
  ...
  { key: 'ten_square_computes_ten_dimensions', assumes: 'FREE_DIMS + COMPACT_DIMS = TEN_DIMS, RAYS = BASE − COINS, compactified count is the rays, free count is the trinity', where: 'src/aura.ts',
  ...
  { key: 'station_ten_is_hexagram_plus_hexbit', assumes: 'STATION_TEN = HEXAGRAM_BITS + HEXBIT_BITS = 10, and STRIP_CHOICES is that ten', where: 'src/hexagram.ts + src/quantum/apps/categories/books/strips.ts',
    live: () => STATION_TEN === HEXAGRAM_BITS + HEXBIT_BITS && STATION_TEN === 10 && STRIP_CHOICES === STATION_TEN },
  { key: 've_faces_are_handle_hexbit_coins', assumes: 'VE_FACES = HANDLE_HEXBITS + HEXBIT_BITS + COINS = 14, and STRIP_LINES is those faces', where: 'src/hexbit/index.ts + src/quantum/apps/categories/books/strips.ts',
    live: () => VE_FACES === HANDLE_HEXBITS + HEXBIT_BITS + COINS && VE_FACES === 14 && STRIP_LINES === VE_FACES },
  { key: 'literature_sonnet_volume', assumes: 'handleBookOf volume is 10^14 occupancy seats from STRIP_CHOICES on STRIP_LINES', where: 'src/quantum/apps/categories/books/strips.ts',
    live: () => {
      const zero = Array.from({ length: STRIP_LINES }, () => 0)
      return handleBookOf(zero).volume === 10 ** 14 && STRIP_LINES === 14 && STRIP_CHOICES === 10
    } },
  { key: 'combinatorial_book_exceeds_handles', assumes: 'the combinatorial book volume strictly exceeds 16^HANDLE_HEXBITS handle doors', where: 'src/quantum/apps/categories/books/strips.ts',
    live: () => {
      const zero = Array.from({ length: STRIP_LINES }, () => 0)
      return handleBookOf(zero).volume > 16 ** HANDLE_HEXBITS
    } },
  { key: 'combinatorial_book_fits_the_uuid', assumes: 'the combinatorial book volume sits strictly inside 2^UUID_BITS addresses', where: 'src/quantum/apps/categories/books/strips.ts',
    live: () => {
      const zero = Array.from({ length: STRIP_LINES }, () => 0)
...
export function axiomHunt(): HuntReport {
  ...
  for (const c of CANDIDATES) {
    const ok = c.live()
    const t = ok ? byKey.get(c.key) : undefined
    if (!ok) refuted.push(c.key)
    else if (!t) {
      exposed.push({
...
if (IS_CLI) {
  ...
  for (const c of CANDIDATES) {
    const state = refutedKeys.has(c.key) ? '✗ REFUTED     ' : exposedKeys.has(c.key) ? '⚠ EXPOSED AXIOM' : '✓ proven      '
