...
const FACTS: Fact[] = [
  ...
  { key: 'speaking_an_address_costs_the_text',
    why: `MESSAGING, COUNTED AT THE WIRE — and the widths being counted are not this repo's to choose: RFC 9562, "Universally Unique IDentifiers (UUIDs)", which obsoletes RFC 4122, fixes a uuid at ${UUID_PAYLOAD_BITS} bits and its printed form at ${UUID_TEXT_CHARS} characters (32 hex digits and 4 hyphens). Naming the RFC says who fixes the spelling so a reader can go and check it; it proves nothing below, because what is sealed here is the arithmetic OVER those widths and the kernel decides arithmetic, never a specification. A uuid spelled as text is ${UUID_TEXT_CHARS} characters = ${UUID_TEXT_CHARS * 8} bits carrying a ${UUID_PAYLOAD_BITS}-bit payload — 44% efficiency (128 of 288 bits), identical for EVERY model in the census, because it is the text's cost, not the model's. The per-window address-carrying capacities are sealed for all ${N} (⌊tokens·${TOKEN_BYTES}/${UUID_TEXT_CHARS}⌋ each). uuidna's own channel skips the text: the channel IS the uuid (128 payload bits per address, receipted).`,
