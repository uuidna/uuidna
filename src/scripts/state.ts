...
import { legalGaps, proseGaps, dryGaps, coherentGaps, absenceGaps, pipeGaps, actionsGaps, vacuousGaps, negationGaps, drainGaps, frozenGaps, foldersGaps, blocksGaps, wordsGaps, countsGaps, linesGaps, scriptsGaps, mirrorGaps, lanesGaps, sourcesGaps} from './one-receipt.js'
...
const finders: [string, number][] = [
  ...
  ['folders', foldersGaps().length], ['blocks', blocksGaps().length], ['scripts', scriptsGaps().length], ['mirror', mirrorGaps().length], ['lanes', lanesGaps().length], ['sources', sourcesGaps().length],
