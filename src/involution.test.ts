...
// ── MERGED IN: the trial-agent swarm (2026-09-01, the captain's "merge tests to speedup, dry cleaning entropy").
//
// These tests lived in src/trial-agent-swarm.test.ts and cost 61.4 s there, almost all of it re-deciding
// statements this file had ALREADY decided moments earlier in a different process. `node --test` runs every file
// in its own process, so the holds() memo — which cut the second census from 63 s to 17 s within one process —
// bought nothing across files. Eight test files touch the involution surface and each paid the cold cost alone.
//
// Merging is the fix that needs no cache and can go stale in no way: the swarm does not call census() at all, it
// pays through holds() on the same expensive statements (exhaustive sorting-network and S4 searches, where ten
// theorems carry 72.9% of the cost), and in one process those verdicts are already in hand. The entropy removed
// is the duplicated EVALUATION, not the tests — every assertion below is the one it made before, unchanged.
