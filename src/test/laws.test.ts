// laws — the standing invariants live IN uuidna and every one is DEMONSTRATED by its gate, not asserted. This test
...
test('uuidna states its laws IN uuidna, and every law holds by recomputation (not by prose)', () => {
  ...
  for (const l of r.laws) {
    ...
    assert.equal(typeof l.holds, 'boolean', 'each law reports a COMPUTED holds, not prose')
