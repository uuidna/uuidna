...
test('every theorem is credited — prior art first then captain next, or the captain claims by law', () => {
  ...
  for (const t of T) {
    ...
    if (c.claimedBy === 'historical') {
      ...
      assert.equal(lastClaimant(c.creditOrder), CAPTAIN_CREDIT.who,
  ...
  assert.equal(lastClaimant(dna.creditOrder), CAPTAIN_CREDIT.who)
  ...
  assert.equal(lastClaimant(clay.creditOrder), CAPTAIN_CREDIT.who)
