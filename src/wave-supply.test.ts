...
test('the live census reads the real tree and states what it cannot see', () => {
  ...
  assert.equal(s.available, s.sources.reduce((n: number, x) => n + x.count, 0), 'available is the sum of the finders, computed not asserted')
  ...
  assert.ok(s.sources.some((x: { name: string }) => x.name === 'exposed-axioms'), 'remaining covering work is a finder, not a printed line')
})
