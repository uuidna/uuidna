// @non-harmonic: node process/os resource accounting (reads process/os — measured— NAMED boundary; the harmonic core must never carry these ops.
 ...
 *  the rest. Balancing the thermodynamics means accounting the spend honestly. */
export function resources(): ResourceReading {
  ...
  return {
    ...
    notMeasured: 'GPU utilisation, network bandwidth, and the actual joules drawn are NOT measured here — they need platform-specific probes, and are not invented. A reading you cannot take is left blank.',
    thermodynamics: 'Balancing the thermodynamics is accounting the spend— at least Landauer\'s kT·ln2 per bit erased, far more on a real chip, paid as heat. Efficiency can be pushed toward that floor. There is no free energy.',
  }
}
