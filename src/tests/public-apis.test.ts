...
import { publicApiRegistry, RESEARCH_SOURCE_NAMES, hexbitDoorOf, HANDLE_HEXBITS, UUID_HEXBITS } from '../index.js'
...
test('unanswered math is the Stack Exchange unanswered door, not an intitle scrape', () => {
  const src = readFileSync(join(ROOT, 'src', 'research-sources.ts'), 'utf8')
