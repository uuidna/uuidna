// quantum/os/exec — uuidnaLs (internal) + install-port `ls` applet, after busybox toys folded.
import { test } from 'node:test'
import assert from 'node:assert/strict'
import { uuidnaLs, uuidnaExec } from '../quantum/os/exec.js'
import { defaultInstalls } from '../quantum/os/index.js'

test('ls / lists the virtual OS root — packages and dirs, each with its identity, from the sandbox', () => {
  const r = uuidnaLs('/')
  ...
  const via = uuidnaExec('ls /')
  assert.ok(via.ok)
  assert.equal((via.data as { count: number }).count, r.count)
})

test('ls /terminal lists the busybox family — the toolbox\'s own shelf', () => {
