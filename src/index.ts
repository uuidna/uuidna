...
export { seedUuid, readSeed, filterSeeds, belongsTo, buildLeanPageSeed, verifySeed, toPayloadDocs, retiredUuid, type SeedStatus, type SeedIdentity, type LeanPageSeed, type PayloadDoc } from './payload-seed.js'
...
// theology — the letters are numerals: gematria, isopsephy and abjad values derived from numeral order by the rank rule,
// each reading beside its control (the sorted letters keep the value and move the lattice station: identity, not meaning).
export { NUMERAL_ORDER, rankValueOf, lettersOf, numeralValueOf, readingOf, THEOLOGY_HONEST, type Script, type ScriptureReading } from './theology/index.js'
...
// the quantum-computer DEMARCATION, developed from an external document read as data
export { QC_AUDIT, EXPOSED_GATE_SET, leanNormalise, EXTERNAL_AUDIT_FINDINGS, auditConformance, quantumClaimCensus, renderAuditConformance, renderQuantumClaimCensus, type AuditVerdict, type AuditFinding, type AuditConformance, type QuantumClaimCensus, type QcMetrics } from './quantum/os/qc/index.js'
export { QC_DOCUMENT, QC_INGREDIENTS, QC_PLANES, QC_PLATFORMS, QC_BUILD_STEPS, QC_CHARACTERISATION, QC_FAULT_TOLERANCE, QC_ROUTE, REFUSED_CORRESPONDENCES, qcArithmetic, qcVerdict, renderQcVerdict, type QcStance, type QcDocument, type QcItem, type QcPlane, type QcPlatform, type QcStep, type QcMetric, type QcStage, type QcFact, type RefusedCorrespondence, type QcVerdict } from './quantum/os/qc/index.js'
// the DOI tag layer — prefix ownership recomputed from the registration agency, prior art tagged by DOI
export { DOI_PREFIXES, CROSSREF_PREFIX_API, DATACITE_PREFIX_API, doiPrefixOf, doiPrefixTag, tagDoi, untaggedOwnPrefixes, doorsMintingUntagged, resolveDoiPrefix, verifyDoiPrefix, verifyDoiPrefixes, ownDoiRecords, doiTagCensus, priorArtByDoi, involuteOutcome, outcomeOf, creditOrderFor, renderDoiPrefixAudit, renderPriorArtByDoi, renderOwnDoiRecords, type DoiAgency, type DoiPrefix, type DoiRecord, type DoiPrefixVerdict, type DoiPrefixAudit, type OwnDoiRecord, type DoiTagCensus, type PriorArtByDoi } from './quantum/os/doi/index.js'
// the scholarly doors — every keyless journal-level and article-level API, with the domain-coverage census
export { JOURNAL_DOORS, JOURNAL_DOORS_ABSENT, JOURNALS_MAILTO, doorById, journalSearch, journalSweep, journalCoverage, renderJournalSweep, renderJournalCoverage, type JournalDoor, type JournalLevel, type JournalRow, type JournalReading, type JournalSweep, type JournalCoverage } from './quantum/os/journals/index.js'
// Zenodo COMMUNITIES — the curation half of the API, and the claim/receipt check a deposit's own metadata cannot do
export { zenodoCommunities, zenodoCommunity, verifyZenodoCommunityClaim, renderZenodoCommunities, renderZenodoCommunity, renderZenodoCommunityClaim, ZENODO_API, ZENODO_COMMUNITIES, ZENODO_PROBE_QUERY, UUIDNA_COMMUNITY, EU_COMMUNITY, NAMED_COMMUNITIES, type ZenodoCommunity, type ZenodoCommunityRecord, type ZenodoCommunitiesResult, type ZenodoCommunityResult, type ZenodoCommunityClaim } from './quantum/os/zenodo/index.js'
