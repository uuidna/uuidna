...
// recomputable by anyone holding the same release. "Port exact Alpine" here means PORT THE INTEGRITY—
// the exact bytes are NAMED and CHECKED. This file lives at src/os/**, the ONE place non-determinism
...
export interface AlpineRelease {
  ...
  rootfsSha256: string  // the digest Alpine PUBLISHED for this rootfs — the external anchor
...
const HONEST =
  ...
  'pure-TS SHA-256, so you can PROVE exactly which upstream bytes you hold. "Port" = port the INTEGRITY' +
  'runtime; the bytes are named and verified. Integrity.'
...
export function verifyAlpineRootfs(bytes: Uint8Array, release: AlpineRelease): RootfsCheck {
  ...
  return {
    ...
    honest: 'Exact-copy proof: uuidna re-hashed the bytes you hold with its own KAT-verified SHA-256 and matched them ' +
      'against the release you pinned. It proves provenance (you hold exactly this Alpine rootfs).',
  }
}

/** fetchAlpineLatest(arch, branch) → the UPSTREAM AUTOMATION: read Alpine's PUBLISHED latest-releases metadata over the
 *  network (Node's built-in fetch, zero npm deps — the response is DATA
 ...
 *  why it lives at the os/ boundary the hard-reject exempts. Best-effort: a down mirror yields null. */
export async function fetchAlpineLatest(arch = 'x86_64', branch = 'latest-stable'): Promise<AlpineRelease | null> {
  try {
    ...
    // the document is content. If the shape ever drifts we return null rather than guess.
...
// these. Naming them is the "all" in "port all Alpine": the integrity of the whole matrix.
 ...
 *  PUBLISHED digest (fetched at this boundary. This ports the
...
export async function portAllAlpine(branch = 'latest-stable'): Promise<AlpineCatalog> {
  ...
  return {
    ...
    honest:
      ...
      'by its PUBLISHED SHA-256 (fetched here. uuidna does ' +
      'NOT boot, link, or execute any of it — the exact bytes of every arch are NAMED and CHECKABLE. A down ' +
      'mirror drops that arch (ported < requested); no digest is ever guessed. Integrity.',
  }
}
