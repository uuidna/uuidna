...
//
// integrity. The hosted MCP is READ-ONLY and DETERMINISTIC — it computes from the ledger and
...
const TOOLS: HttpTool[] = ([
  ...
  { name: 'uuidna_css', description: 'THE DESIGN MATRIX AS ONE SERVED STANDARD, at the edge — every colour and type size computed from the ℤ/9 sequence and the vortex orbit (six rungs: 2 has order 6 in ℤ/9*), none authored. Returns {css,vars,receipt,honest}: render the same receipt or you are rendering a different matrix.',
    inputSchema: { type: 'object', properties: {} },
    run: () => matrixCss() },
  { name: 'uuidna_decide', description: 'THE QUANTUM CALCULATOR at the edge, founded on division by zero — it DISCRIMINATES: a statement the kernel sealed is VERIFIED; a proposition the evaluator can evaluate is EVALUATED_TRUE or EVALUATED_FALSE (2 + 2 = 4 evaluates true, 2 + 2 = 5 false — an evaluation, not a kernel verdict), anything it cannot evaluate is UNVERIFIED, never false; every {input} folds to one lean-green shape {verdict,cites,receipt}: a sealed statement is recognized and cited (the kernel decided it already); fresh arithmetic is evaluated TOTALLY mirroring Lean\'s Nat semantics (x/0 = 0, well-defined — DivByZero.lean; exact BigInt; bounded grammar— TRUE is EVALUATED_TRUE, FALSE is EVALUATED_FALSE; a bare expression computes its exact value; prose goes to the gate, language-blind. The twelfth tool: the compact core.',
