...
import { MCP_CATALOG, callTool } from './mcp.js'   // THE ONE CATALOGUE — the edge subtracts from it, never re-lists it   // the DNA census, recomputed at the edge — gap 39's second party
...
// a derived lookup URL plus school-apis' own whole-name rule, never a fetch), so the edge serves the SAME two tools
...
const SERVER = { name: 'uuidna', version: '0.2.7' }
 ...
 *      await. These are PURE and belong here; they wait on the dispatch becoming async, not on safety.
 *  Conservative on purpose: a tool is absent if it TOUCHES such a module at all, even where the
 *  specific function it calls is pure. Shrinking that is the next honest pass, not a guess made today. */
...
/** The catalogue tools this edge INHERITS — computed, never copied, so the two surfaces cannot drift again. */
 ...
 *  and the edge does not — and it may only ever leave this list, never quietly join it. */
