...
// A standard algorithm implemented transparently — not a novel cipher. Integrity.
import { ADDRESS_BYTES, KEY_BYTES, COINS, HEXBIT_BITS, HANDLE_HEXBITS, UUID_BITS, UUID_HEXBITS } from './hexbit/index.js'

const WORD_BITS = UUID_HEXBITS
export const BLOCK_BYTES = KEY_BYTES * COINS
const OCTET = HEXBIT_BITS * COINS
const rotl = (x: number, n: number): number => ((x << n) | (x >>> (WORD_BITS - n))) >>> 0
...
export function chachaBlock(key: Uint8Array, counter: number, nonce: Uint8Array): Uint8Array {
  const s = new Uint32Array(ADDRESS_BYTES)
  s[0] = CONST[0]; s[1] = CONST[1]; s[2] = CONST[2]; s[3] = CONST[3]
  for (let i = 0; i < HANDLE_HEXBITS; i++) s[HEXBIT_BITS + i] = u32le(key, i * HEXBIT_BITS)
  const counterAt = HEXBIT_BITS + HANDLE_HEXBITS
  s[counterAt] = counter >>> 0
  s[counterAt + 1] = u32le(nonce, 0); s[counterAt + 2] = u32le(nonce, HEXBIT_BITS); s[counterAt + 3] = u32le(nonce, OCTET)
  const w = s.slice()
  ...
  for (let i = 0; i < ADDRESS_BYTES; i++) {
    const v = (w[i] + s[i]) >>> 0
    out[i * HEXBIT_BITS] = v & 0xff; out[i * HEXBIT_BITS + 1] = (v >>> 8) & 0xff; out[i * HEXBIT_BITS + 2] = (v >>> 16) & 0xff; out[i * HEXBIT_BITS + 3] = (v >>> 24) & 0xff
  }
  return out
}
...
export function chacha20(key: Uint8Array, counter: number, nonce: Uint8Array, data: Uint8Array): Uint8Array {
  ...
  for (let i = 0; i < data.length; i += BLOCK_BYTES) {
    const ks = chachaBlock(key, counter + (i / BLOCK_BYTES | 0), nonce)
    for (let j = 0; j < BLOCK_BYTES && i + j < data.length; j++) out[i + j] = data[i + j] ^ ks[j]
  }
  return out
}
...
export function poly1305(msg: Uint8Array, otk: Uint8Array): Uint8Array {
  let r = 0n; for (let i = ADDRESS_BYTES - 1; i >= 0; i--) r = (r << 8n) | BigInt(otk[i])
  r &= 0x0ffffffc0ffffffc0ffffffc0fffffffn
  let s = 0n; for (let i = ADDRESS_BYTES - 1; i >= 0; i--) s = (s << 8n) | BigInt(otk[ADDRESS_BYTES + i])
  const p = (1n << 130n) - 5n
  let acc = 0n
  for (let i = 0; i < msg.length; i += ADDRESS_BYTES) {
    const rem = msg.length - i, n = rem < ADDRESS_BYTES ? rem : ADDRESS_BYTES
    let blk = 0n; for (let j = n - 1; j >= 0; j--) blk = (blk << 8n) | BigInt(msg[i + j])
    blk |= 1n << BigInt(OCTET * n)
    acc = ((acc + blk) * r) % p
  }
  acc = (acc + s) & ((1n << BigInt(UUID_BITS)) - 1n)
  const tag = new Uint8Array(ADDRESS_BYTES)
  for (let i = 0; i < ADDRESS_BYTES; i++) { tag[i] = Number(acc & 0xffn); acc >>= 8n }
  return tag
}

const pad16 = (n: number): Uint8Array => new Uint8Array((ADDRESS_BYTES - (n % ADDRESS_BYTES)) % ADDRESS_BYTES)
const le64 = (n: number): Uint8Array => { const b = new Uint8Array(OCTET); let x = BigInt(n); for (let i = 0; i < OCTET; i++) { b[i] = Number(x & 0xffn); x >>= 8n } return b }
...
/** ChaCha20-Poly1305 AEAD encrypt (RFC 8439 §2.8). Returns ciphertext and a tag ADDRESS_BYTES wide. */
export function aeadEncrypt(key: Uint8Array, nonce: Uint8Array, plaintext: Uint8Array, aad: Uint8Array = new Uint8Array()): { ct: Uint8Array; tag: Uint8Array } {
  const otk = chachaBlock(key, 0, nonce).slice(0, KEY_BYTES)
...
export function aeadDecrypt(key: Uint8Array, nonce: Uint8Array, ct: Uint8Array, tag: Uint8Array, aad: Uint8Array = new Uint8Array()): Uint8Array {
  ...
  let diff = 0; for (let i = 0; i < ADDRESS_BYTES; i++) diff |= t[i] ^ tag[i]
