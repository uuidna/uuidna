...
import { uuidnaLs, uuidnaExec } from './quantum/os/exec.js' // the Alpine-package tools — virtual ls + the busybox executor, never a binary
...
const TOOLS: Tool[] = [
  ...
  { name: 'uuidna_os',
    ...
    run: () => osLayer() },
  { name: 'uuidna_ls',
    description: 'LIST A DIRECTORY OF THE VIRTUAL uuidnaOS — the first Alpine-package tool. Pass {path} (default "/"): returns the entries of the install port at that path — ported packages as files, prefix routes as dirs — each with its uuidna/<name> identity, PUBLISHED Alpine meaning, address and 32 hexbit states, folded to one recomputable receipt. "/" the ported packages, "/terminal" the busybox family, "/core" musl. Does NOT run any binary — the OS never executes (theorem the_os_is_bootable_quantum); uuidna\'s logic, busybox\'s identity. Returns {path,entries,count,receipt,hexbits,sealed,honest}.',
    detail: 'THE FIRST ALPINE-PACKAGE TOOL, and the model for making packages "executable" in the virtual uuidnaOS without ever running a binary (the captain\'s order, 2026-08-23). `ls` lives in busybox on Alpine, so this is that utility\'s LOGIC reimplemented in uuidna and computed INSIDE the booted sandbox: bootOS verifies the 832-state world first — a drifted world lists nothing — then childrenOf walks the install port\'s own routes (the sealed alpine-base closure from the committed mirror), so an entry E is a child of DIR when E\'s route is DIR + one segment; a leaf route is its package, a route that is a prefix of a deeper one is a dir. THE LOAD-BEARING HONESTY (theorem the_os_is_bootable_quantum): uuidnaOS is provenance/integrity, NEVER execution — "executable in the virtual OS" means uuidna\'s OWN pure-TS reimplementation of a utility\'s logic over a VIRTUAL filesystem that is the port\'s routes, never Alpine\'s runtime linked or spawned. The tool\'s LOGIC is uuidna\'s; the tool\'s IDENTITY is the busybox package (coreutils/ls lives in busybox on Alpine). Deterministic and edge-clean (pure, no fetch): the listing folds to a content-addressed receipt, its `sealed` is the booted OS\'s own receipt, and moving one entry moves the receipt. Boundary DECLARED — theorem drift_is_named_or_caught.',
    inputSchema: { type: 'object', properties: { path: { type: 'string', description: 'a virtual-OS path, e.g. "/" (default), "/terminal", "/core"' } } },
    run: (a = {}) => uuidnaLs(String(a.path ?? '/')) },
  { name: 'uuidna_exec',
    description: 'RUN THE BUSYBOX TOOLBOX IN THE VIRTUAL uuidnaOS — the whole ported utility family through ONE door. busybox on Alpine is one binary carrying many applets, so this is one tool carrying many: pass {line} as a command, e.g. "ls /terminal", "cat /core", "which busybox", "stat /trust", "apk list", "apk info busybox", "apk depends alpine-base", "help". The apk family is the package manager\'s READ surface only — list the inventory, show a package by name, query deps forward/back; a WRITE verb (add/del) is refused, a provenance OS installs nothing. Each applet is uuidna\'s OWN pure reimplementation of the utility\'s logic over the virtual filesystem (the install port\'s routes), computed inside the booted sandbox. `cat <route>` reads a package\'s provenance record (id/version/checksum/deps/address); `which <name|route>` walks both ways; `stat` reads metadata from the sealed spec; an applet uuidna does NOT port (rm, sh, …) is refused, never faked or run — a provenance OS deletes and executes nothing (theorem the_os_is_bootable_quantum). Returns {line,applet,args,ok,output,data,receipt,hexbits,sealed,honest}.',
    detail: 'THE WHOLE TOOLBOX PORTED TO MCP THROUGH ONE DOOR (the captain\'s order, 2026-08-24: "Port all to mcp"; lead 129 phase a — the OS\'s ported packages BECOME callable tools). busybox is one binary of many applets, so uuidna ports it as one tool of many applets rather than 25 wire entries — the singularity 85 at the metal: `ls /terminal`, `cat /core`, `which busybox` are the terminal\'s own command lines, now answered by the wire. THE APPLETS are busybox\'s filesystem/inspection family, kept to what a provenance filesystem can HONESTLY answer from the sealed install port: ls (list a virtual dir), cat (a package\'s provenance record at a route), which (name↔route), stat (metadata from the spec), pwd (the one root), echo (folds its text to an address — echo in the quantum OS), help. Each is PURE and TOTAL: a bad path is an honest error line, an unported applet is refused with its name, never a crash and never a faked answer — the controls in the test drive `rm -rf /` and `cat` of a missing route to their honest failures. HONEST SCOPE, load-bearing (theorem the_os_is_bootable_quantum): NOTHING EXECUTES — an applet reads the sealed spec and folds an answer; the tool\'s LOGIC is uuidna\'s, its IDENTITY is the busybox package (uuidna/busybox at /terminal). Deterministic, edge-clean (pure, no fetch), one receipt per run that moves when the run moves. Relates to uuidna_ls (the first applet, now folded in), uuidna_registry (the toolbox+OS as one set), the terminal app (whose command lines these are). Boundary declared — theorem drift_is_named_or_caught.',
    inputSchema: { type: 'object', properties: { line: { type: 'string', description: 'a busybox command line, e.g. "ls /terminal", "cat /core", "which busybox", "help"' } }, required: ['line'] },
