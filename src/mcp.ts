...
import {
  ...
  auditText, auditTranslation, auditBook, bookArticle, linkBookFacts, auditMovie, auditZenodo, auditStandard, beaconAnchor, nistConstant, auditCve, auditDetails,
...
const TOOLS: Tool[] = [
  ...
  { name: 'uuidna_audit_details',
    ...
    inputSchema: { type: 'object', properties: { text: { type: 'string', description: 'the document whose every detail is adjudicated' }, title: { type: 'string' } }, required: ['text'] },
    run: (a) => auditDetails(String(a.text), { title: a.title === undefined ? undefined : String(a.title) }) },
