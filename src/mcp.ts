...
const TOOLS: Tool[] = ([
  ...
  { name: 'uuidna_invitation',
    ...
    inputSchema: { type: 'object', properties: { recompute: { type: 'boolean', description: 'run the sweep now' } }, required: ['recompute'] },
    run: (a = {}) => a.recompute === true ? (invitation(ROOT)) : { kind: 'contract', tool: 'uuidna_invitation', recompute: false, why: 'a sweep over the sealed ledger; call with recompute: true' } },
  { name: 'uuidna_lead_clusters',
    description: 'Which sealed wings the tree\'s open and refuted leads stand around — every cluster computed in ONE pass over the corpus, not one scan per name. Leads are addressed by HANDLE (handleOf(toUuid(text))), so identity is content and the crosslink is handle-to-handle; the vocabulary is the tree\'s own wing names rather than a hand-typed synonym table — the derived-not-typed law, theorem monitoring_the_points_covers_every_crack_by_architecture: a complete set of classes covers what has not been seen yet, where a hand-written list covers only what has. Pass `term` to ask about one name (any word, whether or not a wing carries it). UNANCHORED leads — real work naming no sealed wing — are reported rather than dropped, and a source that could not be parsed is UNREADABLE, never counted as zero.',
  ...
  { name: 'uuidna_read',
    ...
    inputSchema: { type: 'object', properties: { uuids: { type: 'array', items: { type: 'string' }, description: 'uuidna_imprint output' } }, required: ['uuids'] },
  ...
  { name: 'uuidna_pentagram_monographs',
    ...
    inputSchema: { type: 'object', properties: { recompute: { type: 'boolean', description: 'sweep every monograph now' } }, required: ['recompute'] },
    run: (a = {}) => a.recompute === true ? pentagramMonographs() : { kind: 'contract', tool: 'uuidna_pentagram_monographs', recompute: false, why: 'the sweep is minutes over the sealed ledger; call with recompute: true' } },
  { name: 'uuidna_spin',
  ...
  { name: 'uuidna_decrypt',
    ...
    inputSchema: { type: 'object', properties: { sealed: { type: 'object', description: 'uuidna_encrypt output' }, passphrase: { type: 'string', description: 'the passphrase the envelope was sealed with' } }, required: ['sealed', 'passphrase'] },
  ...
  { name: 'uuidna_verify_envelope',
    ...
    inputSchema: { type: 'object', properties: { sealed: { type: 'object', description: 'uuidna_encrypt output' } }, required: ['sealed'] },
  ...
  { name: 'uuidna_open_onion',
    ...
    inputSchema: { type: 'object', properties: { uuids: { type: 'array', items: { type: 'string' }, description: 'uuidna_seal_onion output' }, passphrases: { type: 'array', items: { type: 'string' }, description: 'innermost→outermost, the same passphrases the onion was sealed with' } }, required: ['uuids', 'passphrases'] },
  ...
  { name: 'uuidna_open_chain',
    ...
    inputSchema: { type: 'object', properties: { links: { type: 'array', items: { type: 'object' }, description: 'uuidna_seal_chain output' }, passphrases: { type: 'array', items: { type: 'string' }, description: 'the same passphrases the chain was sealed with, in order' }, genesis: { type: 'string' } }, required: ['links', 'passphrases'] },
  ...
  { name: 'uuidna_contract_open',
    ...
    inputSchema: { type: 'object', properties: { sealed: { type: 'object', description: 'uuidna_contract_seal output' }, terms: { type: 'string', description: 'the terms string the contract was sealed with' } }, required: ['sealed', 'terms'] },
  ...
  { name: 'uuidna_grid',
    ...
    inputSchema: { type: 'object', properties: { recompute: { type: 'boolean', description: 'run the sweep now' },  dimension: { type: 'string', enum: ['bg', 'de', 'fr', 'es', 'ru', 'zh'], description: 'a projected ray' }, wing: { type: 'string', description: 'a ledger wing' } } },
    run: (a = {}) => {
      // one seat is cheap; the whole 432-seat report is the sweep
      if (a.recompute !== true && !(a.dimension && a.wing)) return { kind: 'contract', tool: 'uuidna_grid', recompute: false, why: 'the whole grid is a sweep over the sealed ledger; pass {dimension, wing} for one seat or recompute: true for all' }
  ...
  { name: 'uuidna_aead_encrypt',
    ...
    inputSchema: { type: 'object', properties: { key: { type: 'string', description: '32-byte hex' }, nonce: { type: 'string', description: '12-byte hex' }, plaintext: { type: 'string', description: 'utf-8 text to seal' }, aad: { type: 'string', description: 'optional hex' } }, required: ['key', 'nonce', 'plaintext'] },
  ...
  // ── THE FRACTAL HOLOGRAM (the captain, 2026-09-12): uuidna's MCP includes every subdomain MCP. The lattice is pure;
  //    the fan-out is the one network door, allowlisted to the four hosts. ──
  { name: 'uuidna_hologram',
  ...
  { name: 'uuidna_analytics',
    ...
    inputSchema: { type: 'object', properties: { recompute: { type: 'boolean', description: 'fold the analytics now' } }, required: ['recompute'] },
    run: (a = {}) => a.recompute === true ? quantumAnalytics() : { kind: 'contract', tool: 'uuidna_analytics', recompute: false, why: 'the fold is minutes over the sealed ledger; call with recompute: true' } },
  { name: 'uuidna_decode',
    ...
    run: (a = {}) => {
      if (a.key) {
        const t = theoremFor(String(a.key))
  ...
  { name: 'uuidna_discovery_train',
    ...
    inputSchema: { type: 'object', properties: { recompute: { type: 'boolean', description: 'run the sweep now' },  query: { type: 'string', description: 'a topic' } }, required: ['recompute'] },
    run: (a = {}) => {
      if (a.recompute !== true) return { kind: 'contract', tool: 'uuidna_discovery_train', recompute: false, why: 'a sweep over the sealed ledger; call with recompute: true' }
  ...
  { name: 'uuidna_selftest',
    ...
    inputSchema: { type: 'object', properties: { run: { type: 'boolean', description: 'run every check now' } }, required: ['run'] },
    run: (a = {}) => a.run === true ? mcpSelfTest() : { kind: 'contract', tool: 'uuidna_selftest', run: false, why: 'the self-test runs every zero-argument tool twice; call with run: true' } },
  ...
  { name: 'uuidna_rosetta_legs',
    ...
    inputSchema: { type: 'object', properties: { recompute: { type: 'boolean', description: 'run the sweep now' },  key: { type: 'string', description: 'a sealed theorem key' } } },
    run: (a = {}) => {
      // one key is cheap; the whole distribution is the sweep
      if (a.recompute !== true && !a.key) return { kind: 'contract', tool: 'uuidna_rosetta_legs', recompute: false, why: 'the whole distribution is a sweep over the sealed ledger; pass {key} for one theorem or recompute: true for all' }
...
/** THE SCHEMA IS THE CONTRACT, BOTH HALVES. Required keys must be present, and a present key must carry the TYPE the
 *  schema declares. An external audit (2026-09-12) called uuidna_coprime with {a: "twelve", b: [1]} and was answered
 *  {gcd: 1, coprime: true} stamped CLEAN, and uuidna_merkle_root with a string and was answered "leaves.map is not a
 *  function" — a wrong answer returned confidently, and a JavaScript internal returned as an error. Neither computes
 *  now: the refusal names the argument and the type the tool's own schema declares. One gate, every tool, both
 *  surfaces (mcp-http runs the same function on its own tools). */
export function argsGateOf(name: string, schema: unknown, args: Record<string, unknown>): void {
...
/** Over the surface that SERVES it: the stdio catalogue here, the edge's served list on the edge (an external audit
 *  found the hosted benchmark reporting 257 tools while tools/list returned 223, and naming a tool the edge never served). */
export function mcpBenchmark(catalog: readonly { name: string; inputSchema?: { required?: unknown[]; properties?: Record<string, unknown> } }[] = MCP_CATALOG): McpBenchmark {
  const perTool: ToolUsability[] = catalog.map((t) => {
