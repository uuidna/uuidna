...
const TOOLS: Tool[] = [
  ...
  { name: 'uuidna_os',
    ...
    run: () => osLayer() },
  { name: 'uuidna_exec',
    description: 'ALPINE APPS IN THE VIRTUAL uuidnaOS — one door after toy busybox was folded. Pass {line}: "ls /terminal" or "ls /catalogue" (install-port VFS or full census), "apk list" or "apk list --all" (boot closure or whole catalogue), "apk info|search|depends|rdepends …", "man busybox" (man→app→hexbit), "driver" (pinned netboot/modloop provenance), "device" (host quantum executor), "help". apk is READ-only; man compiles Alpine\'s *-doc / *-man-pages package to 32 hexbits. Folded toys (cat/which/stat/pwd/echo/du) refuse with a pointer to apk/man. Nothing executes (theorem the_os_is_bootable_quantum). Returns {line,applet,args,ok,output,data,receipt,hexbits,sealed,honest}.',
    detail: 'ALPINE APPS ARE THE APPS (post man→app→hexbit + browser-usable). The dedicated uuidna_ls wire tool and the hand-rolled busybox toys (cat/which/stat/pwd/echo/du) are FOLDED — callers use this one door. Remaining applets: ls (install-port routes OR /catalogue census), apk (boot closure or full published catalogue READ surface), man (documentation package → 32 hexbit states; provenance identity, never manpage bytes), driver (pinned netboot/modloop bundle from the committed mirror), device (this host executing the sealed quantum algebra), help. A WRITE apk verb and an unported applet are refused, never faked. Relates to uuidna_registry, uuidna_port, the terminal app. Boundary declared — theorem drift_is_named_or_caught.',
    inputSchema: { type: 'object', properties: { line: { type: 'string', description: 'e.g. "ls /catalogue", "apk list --all", "apk info busybox", "man openssl", "driver", "device", "help"' } }, required: ['line'] },
