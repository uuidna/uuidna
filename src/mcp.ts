...
const TOOLS: Tool[] = [
  ...
  { name: 'uuidna_exec',
    ...
    detail: 'Layer 1 simulation: install-port VFS, full catalogue, session apk add/del, busybox applets over virtual fs + session files. Nothing runs Alpine ELF inside this door — boot is verified hexbit loading. Layer 2 (uuidna_run, stdio only): verify-then-run pinned rootfs bytes on the host. Relates to uuidna_port, uuidna_registry, the terminal.',
  ...
  { name: 'uuidna_run',
    ...
    inputSchema: { type: 'object', properties: { command: { type: 'string', description: 'shell command to run after verify' }, spawn: { type: 'boolean', description: 'if true, spawn the recipe (default false — plan only)' } }, required: ['command'] },
    run: async (a = {}) => {
      const { runAlpineCommand } = await import('./os/runtime/index.js')
      return runAlpineCommand(String(a.command ?? ''), { spawn: a.spawn === true })
    } },
  { name: 'uuidna_port',
