-- lean/Quantum.lean — GENERATED. The QUANTUM computer — the exact facts the classical state-vector simulator (src/quantum.ts) computes: the Born rule on the Bell state, no-signaling marginals, superposition, GHZ(3), the gate truth-tables (CNOT, Toffoli, SWAP), and the phase-gate algebra (S·S=Z, Z²=I, S·S†=I). HONEST SCOPE: the algebra of a CLASSICAL simulation on integer positions — 2^n amplitudes, exponential, NO quantum advantage, NOT quantum hardware. Every proof `by decide`, sorry-free, no Mathlib. 0/7.
...
theorem cnot_truth_table : ((List.range 4).map (fun i => i ^^^ (2 * (i % 2)))) = [0,3,2,1] := by decide

-- CNOT is its own inverse: applying it twice returns every basis state — a reversible (unitary) permutation
theorem cnot_involution : (List.range 4).all (fun i => (let j := i ^^^ (2 * (i % 2)); j ^^^ (2 * (j % 2))) == i) := by decide

-- Toffoli (CCX) flips q2 iff q0 ∧ q1 — the reversible classical AND: i ↦ i ⊕ 4·(q0·q1) = [0,1,2,7,4,5,6,3] on three qubits
theorem toffoli_truth_table : ((List.range 8).map (fun i => i ^^^ (4 * ((i % 2) * (i / 2 % 2))))) = [0,1,2,7,4,5,6,3] := by decide
...
theorem pauli_x_involution : (List.range 2).all (fun i => (i ^^^ 1) ^^^ 1 == i) := by decide
...
theorem toffoli_involution : (List.range 8).all (fun i => (let j := i ^^^ (4 * ((i % 2) * (i / 2 % 2))); j ^^^ (4 * ((j % 2) * (j / 2 % 2)))) == i) := by decide
