...
/-- THE WORKHORSE IS PROVED, NOT TRUSTED. Every obstruction in this wing is stated through pmod, so a wrong pmod
    ...
    every modulus from 3 to 30. THE DOMAIN IS THE FULL ONE, AND IT WAS BRIEFLY NOT: at exponent 30 the left side
    reaches 29^29, about 10^42, and the independent evaluator refused it with an overflow. The first answer was
    to narrow the exponent to 11 so the value stayed inside double precision, which is shrinking a claim to fit
    an instrument. The instrument was already exact: it carries BigInt powers and a fused modular
    exponentiation, and only a parenthesised power was being forced to a scalar before the modulus could reduce
    it. The modulus of a big integer is small; only the intermediate was ever large. Reducing exactly costs
    nothing and leaves the full domain standing: the cases the ledger records for it decided by the same kernel
    that decides the obstructions. Two earlier implementations were replaced on COST, never on meaning — a^n % m
    built the whole power before reducing, and naive repeated multiplication took n steps where lambda(107) =
    106 puts 1.19 million multiplications in one theorem — and this theorem is what makes that substitution
...
/-- THE SEARCH ITSELF. For every exponent in [3,4,5,6], the window 1 <= x <= y < z <= 20 contains NO solution of
    x^n + y^n = z^n — four exhaustive walks, every triple decided by the kernel. WHAT THIS IS NOT: it holds for
    the triples inside the window and says nothing about any triple outside it, and the near-miss below is why
    that silence must be taken seriously rather than waved through. -/
