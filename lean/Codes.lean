-- lean/Codes.lean — GENERATED. THE ERROR-CORRECTING CODES — Hamming(7,4), the perfect-code sphere-packing, distance/correction bounds, and the XOR checksum, decidable. Every proof `by decide`, sorry-free, no Mathlib.
...
theorem xor_checksum_catches_flip : (List.range 8).all (fun a => (a ^^^ 5) ^^^ a == 5) := by decide
