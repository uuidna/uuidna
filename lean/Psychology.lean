-- lean/Psychology.lean — GENERATED. THE INSTRUMENT — the ARITHMETIC of psychology's instruments and named models (the Likert midpoint, the Big Five count, Miller's span, Hick's bits, the detection table, the Weber–Fechner ladder, Dunbar's rounded layers) and ONLY that. Every proof `by decide`, sorry-free, no Mathlib, and axiom-free — depends on NO axiom beyond the leanprover/lean4 kernel (verified by scripts/lean-axioms; not even propext).
