-- lean/Psychology.lean — GENERATED. THE INSTRUMENT — the ARITHMETIC of psychology's instruments and named models (the Likert midpoint, the Big Five count, Miller's span, Hick's bits, the detection table, the Weber–Fechner ladder, Dunbar's rounded layers) and ONLY that. NOT a claim about the mind, behaviour, emotion, personality, or any diagnosis — uuidna is not a clinician. Every proof `by decide`, sorry-free, no Mathlib.
