...
/-- A STORE'S COST TO ITS HOST IS COUNTED IN INODES, NOT RECORDS. Leaving out this wing's own 3 records, 71,592
    leaves sit under 256 + 43,527 + 71,434 + 71,592 folders across the 4 levels, so with the root the store
    occupies 258,402 inodes — more than three for every record it holds. The floor is two by construction: the
...
theorem the_store_footprint_is_its_folders : (71592 + 256 + 43527 + 71434 + 71592 + 1 = 258402) ∧ (258402 > 3 * 71592) := by decide

/-- THE CONTROL: SHARING DOES OCCUR ABOVE THE LEAF. Handles 00bc4bbe and 00bc4bc1 are two records of this store
    with one third-level folder, 00bc4b, and at that level 71,434 folders hold 71,592 leaves — which shows the
    footprint is a measurement of this store and not a constant two-per-record identity that would pass whatever
    the store held. -/
theorem the_third_level_already_shares : ([0,0,11,12,4,11,11,14].take 6 = [0,0,11,12,4,11,12,1].take 6) ∧ ([0,0,11,12,4,11,11,14] ≠ ([0,0,11,12,4,11,12,1] : List Nat)) ∧ (71434 < 71592) := by decide
