-- lean/HandleSpan.lean — GENERATED. THE HANDLE SPAN — what 65536 handles of 32 qubits each come to, and what that total is NOT. A handle is the uuid's first segment: 8 hex characters at 4 bits each, 32 bits spanning 2^32 = 4294967296 addresses. Take 2^16 = 65536 of them and the qubit counts ADD while the spans multiply — 2^16 * 2^5 = 2^21 — so the total is 2097152 qubits, which is 65536 * 32 read the other way. The derivation is exactly that exponents add: 16 + 5 = 21, and NOT 16 * 5 = 80, which the seal proves rather than assumes. WHAT IT IS NOT, sealed on the lines and not merely disclaimed: a register of n qubits holds 2^n amplitudes, so 16 qubits is already 65536 complex numbers held at once (the shipped MAX_MESSAGE_QUBITS), and the 2097152-qubit total is strictly greater than that and not equal to it. A total reached by adding exponents describes what can be NAMED, never what can be HELD. The amplitudes such a register would carry is 2 raised to 2097152 — named here, never evaluated, and no claim is made that it can be realised. PURE ARITHMETIC: every number is a power of two or a product of two of them, no ledger count appears, and nothing is measured from the world. HONEST SCOPE: integrity, not truth. Every proof `by decide`, sorry-free, no Mathlib, and axiom-free — depends on NO axiom beyond the leanprover/lean4 kernel (verified by scripts/lean-axioms; not even propext).
