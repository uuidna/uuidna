...
/-- The coins' EXCHANGE RATE is the TRAITOR'S TOKEN COST: a coin is worth exactly what it costs to FORGE it. A
    ...
    the crew MINT for 2: forgery NEVER pays, and the coin is BACKED by the cost to counterfeit it. HONEST: 2¹²⁸
    is a BOUND set by SHA-256, not a maximum, and it is the COLLISION-RESISTANT fold's cost — the fast FNV
    receipt is tamper-evident but not collision-resistant; add a key (HMAC) and forgery also needs the secret. -/
...
/-- THE SUPERPOSITION CLAIM — the credit law at its full extent: the captain claims the unclaimed, and the
    ...
    any collapse stays exactly two (110 − 108 = 2). HONEST SCOPE: the claim is of ROOM, never of truth — a
    claimed superposition is claimed capacity, and its collapse still pays the two coins and passes the trial;
    claiming the space solves nothing (coins_compute_but_solve_none stands over this claim as over every other). -/
...
theorem coin_and_heart_generate_the_scales : (2*5 = 10) ∧ (2+5 = 7) ∧ (2^5 = 32) := by decide
