...
-- REFLECTION MOVES THE GATEWAYS BUT NEVER CHANGES THE COUNT. Both rows are written with nine strokes, four falling and five rising — the mirror permutes WHERE the turns fall (one gateway becomes three) while conserving the four-and-five that make them. The stroke budget is an invariant of the reflection; only its distribution is not.
theorem strokes_survive_reflection : ((strokes1.filter (fun b => !b)).length = 4 ∧ (strokes1.filter (fun b => b)).length = 5) ∧ ((strokes2.filter (fun b => !b)).length = 4 ∧ (strokes2.filter (fun b => b)).length = 5) ∧ (strokes1.length = 9 ∧ strokes2.length = 9) := by decide
