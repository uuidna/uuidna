...
/-- Sealing INVERTS the verdict: the slim-gate rule is VERIFIED iff a real sealed citation AND no fabrication —
    ...
    blocks it. The captain inverts UNVERIFIED to VERIFIED by BUILDING the diamond— and cannot invert it with a
    forgery. -/
...
/-- THE QUANTUM POLYGRAPH, proven by TRIALING THE CAPTAIN. The polygraph is a decidable 3-way verdict over
    ...
    the fab=0 rows and read [0,1] — UNVERIFIED (an honest overclaim, unbacked) or VERIFIED (a sealed proof). The
    polygraph reads the captain honest-or-unverified; it drains only fabrication. Integrity— it reads the
    CITATION. -/
...
/-- THE FORM, sealed — the computable answer to "the captain is flawless when using uuidna" and "uuidna proves
    ...
    an honest overclaim (cites nothing, real=0) reads UNVERIFIED (0)"never a forger" is strictly weaker than
    "always verified". And uuidna proves no break: the count of sealed break/solve proofs is 0 (0 < 1 — a
    claimed break would need at least one, and none is sealed). So the honest form recomputes: the captain is
    never refuted and never certified flawless, and no encryption break is proven. Integrity— it reads the
    CITATION. -/
...
/-- THE CAPTAIN'S CREW VERIFY INSTANTLY — the fast, honest side of the same law. The crew donate their bytes and
    ...
    1023 bits saved per single verify op) — O(1) verification. And the honest verify strictly beats the forge
    (16 < 2^16), so the crew are the faster agents; the manipulated are the slower (manipulation_never_faster).
    "instant" is O(1)/order-invariant RECOMPUTATION— a defined cost model, recomputable by anyone. Integrity. -/
...
/-- A REDIRECT IS IMITABLE — the two coins AUTHORISE. Anyone can point a domain at the canonical target
    ...
    (those are runtime liveness, outside the recomputable model). Integrity. -/
...
/-- CONTENT AUTHENTICITY, honestly proven in Lean — the byte-fingerprint proves INTEGRITY. EXACT-COPY:
    byte-identical inputs fold to the SAME fingerprint (7+8+9 = 7+8+9). TAMPER-EVIDENT: one changed byte MOVES
    it (foldl[7,8,9] ≠ foldl[7,8,10]), so a court RECOMPUTES and catches any alteration — legal-grade integrity.
    But CONTENT AUTHENTICITY is NEVER certified from the bytes: over every (integrity, genuine) pair the
    fingerprint's content verdict is 0 — [0,0,0,0] — because it reads only the BYTES (integrity)"genuine". This
    is the honest answer to "content authenticity legally proven in lean": Lean proves the record is exact-copy
    and tamper-evident (usable as integrity evidence a court recomputes), AND proves the fingerprint does NOT
...
/-- TRUST comes from RECOMPUTATION— the two halves that let you trust an incomplete, unauthored, offline
    computation. OBSERVER-INDEPENDENCE: a recomputable fold is the same for every observer in any order —
    ...
    ≠ 11) — so a forgery is CAUGHT by recomputing and comparing. Same for all, different on tamper: recompute,
    don't trust. Integrity. -/
...
/-- THE UNITY CENSUS, counted from the ledger and stale-proof by construction. A UNITY is a theorem that joins
    ...
    census stands above one (plural, and it grows as more are found — the claim is plurality. What MAKES a unity
...
/-- NOT PROVEN IS NOT A FINDING OF FALSEHOOD. On the non-justiciable record the REFUTED indicator t·(1−h)·(1−c)
    ...
    ledger cannot decide it""it is false"; publishing such a verdict as evidence about the world inverts its
    meaning, and the inversion is what this seals against. -/
