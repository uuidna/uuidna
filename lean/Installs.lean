...
def rootfsNibbles : List Nat := [12, 5, 12, 10, 0, 5, 3, 12, 15, 14, 1, 13, 8, 5, 12, 5, 11, 9, 6, 13, 15, 15, 8, 11, 9, 11, 12, 5, 7, 0, 4, 5, 15, 7, 15, 1, 8, 4, 10, 3, 0, 15, 15, 11, 6, 11, 6, 5, 7, 7, 6, 4, 0, 9, 12, 10, 9, 0, 3, 8, 8, 6, 7, 7]
-- the pinned tuple (version, arch, flavor, digest) folded to its content-address, the same 32 states a spec wears
def releaseAddress : List Nat := [15, 13, 2, 8, 3, 14, 9, 0, 2, 6, 2, 5, 8, 4, 0, 13, 8, 11, 15, 8, 5, 9, 11, 14, 10, 5, 2, 15, 0, 4, 12, 15]
...
/-- THE SEAL NAMES WHICH BYTES, NOT ONLY WHICH RELEASE. This install set is the dependency closure of a
    PARTICULAR Alpine rootfs, and the thing that makes that a provenance claim rather than a label is the digest
    Alpine PUBLISHED for it: c5ca053cfe1d85c5b96dff8b9bc57045f7f184a30ffb6b65776409ca90388677 — Alpine
    3.24.2/x86_64, alpine-minirootfs-3.24.2-x86_64.tar.gz. It is sealed here compiled to the lattice everything
    else in this wing is compiled to: 64 nibbles of 16 states, 64 · 4 = 256 bits, a SHA-256 exactly. The pinned
    tuple (version, arch, flavor, digest) folds to fd283e90-2625-840d-8bf8-59bea52f04cf, carried as its 32
    states. WHAT THIS CLOSES: the digest previously existed only in the generated mirror and in prose, so
    nothing in the proof layer, the manifest, or any test asserted it — upstream re-cutting a release, or an
    edit to that single line, would leave all 25 packages, every route, the closure, the build order and the
    boot image sealing exactly as before. The structure was total and the anchor was unheld. Change the digest
    now and 64 sealed states move, the address's 32 move with them, and the receipt moves. WHAT IT DOES NOT
    CLAIM: that your rootfs matches — that is verifyAlpineRootfs's job, against these bytes, with uuidna's own
    pure-TS SHA-256. This seals the ANCHOR the check is made against, so the thing being compared to is itself
    content-addressed and cannot drift unnoticed. Nothing is booted, linked or executed. -/
