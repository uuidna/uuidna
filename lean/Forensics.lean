...
/-- A GUESSED ADDRESS DOES NOT LAND. The ledger holds 71,022 distinct addresses, every one a v8 uuid with 122
    free bits, so a forger who invents an address and frames it as sealed hits a real one with odds below
    2^-105: 71,022 × 2^105 < 2^122. forensics() flags every miss as a false address — run here on a forged
...
theorem forged_address_odds_are_negligible : 71022 * 2 ^ 105 < 2 ^ 122 := by decide

/-- A GUESSED HANDLE RARELY LANDS EITHER. The handle store holds 71,626 leaves in a 32-bit handle space, so a
    guessed handle names an occupied leaf with odds below 2^-15: 71,626 × 2^15 < 2^32. The odds are far weaker
...
theorem forged_handle_odds_are_small : 71626 * 2 ^ 15 < 2 ^ 32 := by decide

/-- THE CONTROL: THE BOUND CAN FAIL. With 16-bit handles the store's 71,626 leaves would outnumber the 65,536
...
theorem sixteen_bit_handles_would_collide : 71626 > 2 ^ 16 := by decide
