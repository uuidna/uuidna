-- lean/Affine.lean — GENERATED. AGL(1,ℤ/9), ENUMERATED — the group OneLeap.lean names and never lists. An affine map is x ↦ ax + b with a a UNIT of ℤ/9, so six multipliers by nine offsets give fifty-four elements, listed here rather than counted. The six units are exactly the residues with a multiplicative inverse mod nine — 1, 2, 4, 5, 7, 8 — and three, six and zero are excluded because a non-unit multiplier gives a map that cannot be undone. CLOSURE is decided over all 54 × 54 = 2916 ordered products: composing any two gives one of the fifty-four, which is the group axiom a count can never establish. The identity x ↦ 1·x + 0 fixes every element from both sides, and every map has an inverse inside the set. AND IT IS NOT ABELIAN, shown by exhibiting the failure: 2376 of the 2916 ordered pairs do not commute and only 540 agree, so order matters almost everywhere. Elements are packed one per Nat as e = a·9 + b, so the kernel unfolds no pairs or structures. PURE ARITHMETIC, no ledger count, nothing measured from the world. HONEST SCOPE: integrity, not truth — this decides the finite group, never any claim about what it models. Every proof `by decide`, sorry-free, no Mathlib, and axiom-free — depends on NO axiom beyond the leanprover/lean4 kernel (verified by scripts/lean-axioms; not even propext).
