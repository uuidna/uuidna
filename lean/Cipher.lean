-- lean/Cipher.lean — GENERATED. CRYPTO ∩ DNA — the shared algebra of ciphers and the strand, and its HONEST limits: base-pairing is a fixed-key XOR (a one-time-pad step), the pad is self-inverse but key reuse leaks the plaintext XOR, a linear fold is malleable (a receipt is integrity, not a seal), the transport leaks message length, translation is lossy (never a cipher), an affine S-box is invertible but linear, and Grover only halves the key (256→128). HONEST SCOPE: these are the DECIDABLE BOUNDS of the algebra — what it guarantees and what it cannot; secrecy itself is ChaCha20-Poly1305, not this. Every proof `by decide`, sorry-free, no Mathlib.
...
theorem complement_is_xor_key3 : (List.range 4).all (fun x => 3 - x == x ^^^ 3) := by decide
...
theorem otp_self_inverse : (List.range 16).all (fun m => (List.range 16).all (fun k => (m ^^^ k) ^^^ k == m)) := by decide
...
theorem otp_key_reuse_leaks_xor : (List.range 8).all (fun m1 => (List.range 8).all (fun m2 => (List.range 8).all (fun k => ((m1 ^^^ k) ^^^ (m2 ^^^ k)) == (m1 ^^^ m2)))) := by decide
...
theorem xor_fold_is_malleable : (List.range 16).all (fun a => (List.range 16).all (fun d => (a ^^^ d) ^^^ a == d)) := by decide
