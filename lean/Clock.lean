-- lean/Clock.lean — GENERATED. THE CLOCK WITHOUT A NOW — the step algebra src/quantum/clock computes, decided. Reading an oscillator is the largest source of non-determinism there is: the same computation would answer differently on two machines, and every receipt in this system rests on the opposite promise, which is why the harmonic scan hard-rejects wall-clock reads. So time here is a POSITION IN A SEQUENCE and never a reading — steps are counted, the way unitary evolution is counted rather than timed. Sealed: the residue of a step is its place in the doubling orbit 2^k mod 9, reading 1, 2, 4, 8, 7, 5 and then the same six again, so the clock's ring is finite though its step count is not; the clock RETURNS TO ITS RESIDUE WITHOUT RETURNING TO ITS STEP, since step 0 and step 6 share a residue and are not the same step; a distance is a COUNT and not a duration, symmetric and zero exactly when the positions coincide; advancing moves strictly forward, so the step is an odometer and not a dial; and of any two positions exactly one of earlier, later or same holds — the trichotomy that is everything a clock without a now can say, and enough to order a computation. AND THE RESIDUE IS A FUNCTION OF THE STEP ALONE, so two machines computing the same step agree without consulting anything outside the arithmetic. PURE ARITHMETIC, nothing measured from the world — which is the subject, not merely the method. HONEST SCOPE: integrity, not truth — this decides an ordering of computation, never a measurement of duration, never a wall time, and never a claim about physical clocks. Every proof `by decide`, sorry-free, no Mathlib, and axiom-free — depends on NO axiom beyond the leanprover/lean4 kernel (verified by scripts/lean-axioms; not even propext).
