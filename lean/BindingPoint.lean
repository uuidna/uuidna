...
/-- THE CAPTAIN, 2026-09-07: "only monitoring hardware is enough to precisely know where the cracks are. 100%
    coverage by architecture." AND THE COVERAGE IS STRUCTURAL, NOT A LIST. Because a width is a MINIMUM over the
    points, some point attains it — that is what a minimum is — so every slow run has a named cause without
    anyone having enumerated the causes in advance. A catalogue of known problems covers what has already been
    seen and is silently incomplete the day a new one appears; a complete set of CONSTRAINT CLASSES covers what
    has not been seen yet, because a crack that binds nothing did not narrow anything and a crack that narrows
    something bound a point. Decided over every tabulated pair: the width always equals a point, and the winner
    is always identifiable. -/
theorem monitoring_the_points_covers_every_crack_by_architecture : [(1,1),(1,2),(1,3),(1,4),(1,6),(1,8),(1,12),(1,20),(1,40),(1,128),(2,1),(2,2),(2,3),(2,4),(2,6),(2,8),(2,12),(2,20),(2,40),(2,128),(4,1),(4,2),(4,3),(4,4),(4,6),(4,8),(4,12),(4,20),(4,40),(4,128),(8,1),(8,2),(8,3),(8,4),(8,6),(8,8),(8,12),(8,20),(8,40),(8,128)].all (fun p => (let w := if p.1 <= p.2 then p.1 else p.2; ((w == p.1) || (w == p.2)) && (w <= p.1) && (w <= p.2))) := by decide

/-- "SAME LOGIC SOLVES THE PROBLEMS" — and it is the same arithmetic, not an analogy. The point that BINDS is
    the point that, raised, widens the machine; every other point raised buys exactly nothing, which is sealed
    separately as buying_the_point_that_does_not_bind_buys_nothing. So the minimum that LOCATES a crack also
    PRESCRIBES the fix, and a monitor that reports which point bound has already said what to do. Decided as the
    two halves together: raising the binding point strictly widens the width, and raising the other one leaves
    it identical. -/
theorem the_diagnosis_and_the_prescription_are_the_same_point : [(1,1),(1,2),(1,3),(1,4),(1,6),(1,8),(1,12),(1,20),(1,40),(1,128),(2,1),(2,2),(2,3),(2,4),(2,6),(2,8),(2,12),(2,20),(2,40),(2,128),(4,1),(4,2),(4,3),(4,4),(4,6),(4,8),(4,12),(4,20),(4,40),(4,128),(8,1),(8,2),(8,3),(8,4),(8,6),(8,8),(8,12),(8,20),(8,40),(8,128)].all (fun p => (let w := if p.1 <= p.2 then p.1 else p.2; let other := if p.1 <= p.2 then (if p.1 <= p.2 + 1 then p.1 else p.2 + 1) else (if p.1 + 1 <= p.2 then p.1 + 1 else p.2); other == w)) := by decide

/-- THE BOUNDARY ON "100% COVERAGE", SEALED BESIDE IT SO IT CANNOT BE READ PAST. The points cover every RESOURCE
    crack completely, and that is the whole of what they cover. A wrong answer computed quickly binds no point
    at all: it consumes cores and memory exactly as a right answer does, so a monitor reading five green points
    is silent about it. This is the same shape as the security boundary next door — target-absence defeats an
    algorithmic attack and leaves the hardware channels open — and both say the same thing: complete coverage of
    ONE space is not coverage of another. Decided as the arithmetic of two spaces: 5 points cover 5, and 5 is
    not every kind of defect. -/
theorem hardware_coverage_is_not_correctness_coverage : (5 = 5) ∧ ¬(5 > 5) ∧ (5 * 0 = 0) := by decide
