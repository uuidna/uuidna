...
- **`607701a3`** Quantum advantage or speedup claims
  <br><small>already gated by the project: next.ts fails a release on such a title, and a test asserts the prose drains. No unbacked claim exists — the opposite is enforced, and the sealed boundary is [`n_qubit_dimension`](/theorem/n_qubit_dimension): 2ⁿ counts the classical simulation COST, never a speedup.</small>
