...
> THE MATCHING — connecting people as decidable arithmetic: the handshake lemma, mutual (symmetric) choice, fixed-point-free pairings and the bounded cost of stable matching. NOT a dating service, NOT anyone's data — only the graph theory a matching rests on.

**8 theorems**, each proven `by decide` in [lean/Matching.lean](/lean/Matching.lean), axiom-free against the bare Lean kernel. This article is computed from the ledger — nothing here is authored; every claim carries its citation.
