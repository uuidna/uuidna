...
**12 theorems**, each proven `by decide` in [lean/Solids.lean](/lean/Solids.lean), axiom-free against the bare Lean kernel. This article is computed from the ledger — nothing here is authored; every claim carries its citation.
...
integrity, not truth — each theorem seals its exact decidable arithmetic, nothing beyond.
:::
