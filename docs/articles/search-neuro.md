...
asked in parallel about *The algebra of the neuron* — the wing sealed in [lean/Neuro.lean](/lean/Neuro.lean) with **9 theorems**. Each
...
- [all_or_none](/theorem/all_or_none) — `(List.range 10).all (fun x => (if x >= 5 then 1 else 0) == 0 || (if x >= 5 then 1 else 0) `
...
- [firing_monotone](/theorem/firing_monotone) — `(List.range 9).all (fun x => (if x >= 5 then 1 else 0) <= (if x + 1 >= 5 then 1 else 0))`
- [spatial_summation](/theorem/spatial_summation) — `((if 3 >= 5 then 1 else 0) = 0) ∧ ((if 3 + 3 >= 5 then 1 else 0) = 1)`
- [excitatory_inhibitory_net](/theorem/excitatory_inhibitory_net) — `((3 - 1 : Int) = 2) ∧ ((3 - 3 : Int) = 0)`
- [action_potential_swing](/theorem/action_potential_swing) — `((40 - (-70) : Int) = 110) ∧ ((-70 : Int) < -55) ∧ ((-55 : Int) < 40)`
- [hebbian_coincidence](/theorem/hebbian_coincidence) — `(List.range 2).all (fun a => (List.range 2).all (fun b => (a * b == 1) == (a == 1 && b == `
- [refractory_caps_spike](/theorem/refractory_caps_spike) — `(if 1 < 2 then 0 else (if 9 >= 5 then 1 else 0)) = 0`
