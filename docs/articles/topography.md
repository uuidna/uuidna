...
**18 theorems** and **416 decided cases**, from [contour_index_every_fifth](/theorem/contour_index_every_fifth) onward, each proven `by decide` in <a href="/lean/Topography.lean">lean/Topography.lean</a>, axiom-free against the bare Lean kernel. The case count is what the generator's own walk visited while computing the facts — the ledger's tally, never a number typed into prose. This article is computed from the ledger — nothing here is authored, and every claim carries its citation. 9 of its 18 theorems seal a BOUNDARY rather than a capability — naming what the model does not do, where it fails, or what it excludes — starting with [contour_index_every_fifth](/theorem/contour_index_every_fifth). A boundary stated here is decided.
