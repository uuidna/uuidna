...
| `a24a4222` | zenodo.org | zenodo record 18817015: The Coins of Al-Andalus: Evolution and Historical Context | UNVERIFIED | VERIFIED |
...
**24 findings · 24 usable search-trial combinations · receipt `9d988597`** (fold of every finding's address — recompute by re-running the search).

The sealed backing this trial held the findings beside:

- [sailing_books_extract_decidable_facts](/theorem/sailing_books_extract_decidable_facts) — `(List.range 5).length = 5`
- [sailing_address_is_128_bits_in_36_chars](/theorem/sailing_address_is_128_bits_in_36_chars) — `(32 * 4 = 128) ∧ (32 + 4 = 36)`
- [sailing_extraction_deterministic](/theorem/sailing_extraction_deterministic) — `((List.range 5).map (fun n => n * n)) = [0,1,4,9,16]`
- [sailing_book_receipt_order_invariant](/theorem/sailing_book_receipt_order_invariant) — `(List.foldl (fun a b => a + b) 0 [1,2,3] = List.foldl (fun a b => a + b) 0 [3,1,2]) ∧ (Lis`
- [sailing_weather_match_decides_both_ways](/theorem/sailing_weather_match_decides_both_ways) — `((5 * 2 == 10) = true) ∧ ((5 * 2 == 11) = false)`
- [sailing_distinct_facts_distinct_addresses](/theorem/sailing_distinct_facts_distinct_addresses) — `((List.range 7).map (fun n => (n * 3) % 7)) = [0,3,6,2,5,1,4]`
- [sailing_corroboration_and_divergence_partition](/theorem/sailing_corroboration_and_divergence_partition) — `(([5,5,7].filter (fun x => x == 5)).length = 2) ∧ (([5,5,7].filter (fun x => x != 5)).leng`
- [sailing_weather_verification_open](/theorem/sailing_weather_verification_open) — `(100 / 10 = 10)`
- [sailing_cross_book_resonance_match](/theorem/sailing_cross_book_resonance_match) — `(([42,7,42].filter (fun x => x == 42)).length = 2) ∧ (([42,7,42].filter (fun x => x == 7))`
...
- [sailing_cluster_coherence_decidable](/theorem/sailing_cluster_coherence_decidable) — `(((2 * 5 == 10) && (10 / 2 == 5)) = true) ∧ (((2 * 5 == 10) && (10 / 2 == 6)) = false)`
