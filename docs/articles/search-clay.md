...
asked in parallel about *The seven reflected* — the wing sealed in [lean/Clay.lean](/lean/Clay.lean) with **17 theorems**. Each
...
**24 findings · 24 usable search-trial combinations · receipt `3511d53c`** (fold of every finding's address — recompute by re-running the search).

The sealed backing this trial held the findings beside:

- [clay_reflection_involution](/theorem/clay_reflection_involution) — `(List.range 10).all (fun x => dz (dz x) == x)`
- [clay_reflection_fixed_points](/theorem/clay_reflection_fixed_points) — `((List.range 10).filter (fun x => dz x == x)) = [0, 5]`
...
- [clay_status_dna_total](/theorem/clay_status_dna_total) — `(6 + 1 = 7) ∧ ((6:Nat) < 7) ∧ ((1:Nat) ≤ 7)`
- [clay_collision_law](/theorem/clay_collision_law) — `((List.range 8).filter (fun p => p % 2 == 1 && (p / 2) % 2 == 1 && (p / 4) % 2 == 1)).leng`
- [clay_launder_refused](/theorem/clay_launder_refused) — `(2 * 7 + 1 = 15) ∧ ((0:Nat) < 15) ∧ (15 - 15 = 0)`
- [clay_law_vote_theorems_only](/theorem/clay_law_vote_theorems_only) — `(7 + 0 = 7) ∧ ((0:Nat) < 7) ∧ ((7:Nat) > 0)`
