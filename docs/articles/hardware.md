...
**14 theorems**, each proven `by decide` in [lean/Hardware.lean](/lean/Hardware.lean), axiom-free against the bare Lean kernel. This article is computed from the ledger — nothing here is authored; every claim carries its citation.
...
integrity, not truth — uuidna SEALS this spec so a gate design can be verified against it; it does NOT fabricate a device, synthesise a netlist, or develop silicon.
:::
