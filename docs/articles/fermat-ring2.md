...
**58 theorems** and **4,229,490 decided cases**, from [unit_group_exponent_mod_4](/theorem/unit_group_exponent_mod_4) onward, each proven `by decide` in <a href="/lean/FermatRing2.lean">lean/FermatRing2.lean</a>, axiom-free against the bare Lean kernel. The case count is what the generator's own walk visited while computing the facts — the ledger's tally, never a number typed into prose. This article is computed from the ledger — nothing here is authored, and every claim carries its citation. 41 of its 58 theorems seal a BOUNDARY rather than a capability — naming what the model does not do, where it fails, or what it excludes — starting with [unit_group_exponent_mod_4](/theorem/unit_group_exponent_mod_4). A boundary stated here is decided.
