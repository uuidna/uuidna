...
**6 theorems**, each proven `by decide` in [lean/Glagolitic.lean](/lean/Glagolitic.lean), axiom-free against the bare Lean kernel. This article is computed from the ledger — nothing here is authored; every claim carries its citation.
...
the documented numeral arithmetic and the seven-fold symmetry; the rosette's MEANING (the seven planets, the days, the Dulo dynasty, a sun-sign) is historically DEBATED, not decoded here.
:::
