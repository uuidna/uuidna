...
**8 theorems**, each proven `by decide` in [lean/Optics.lean](/lean/Optics.lean), axiom-free against the bare Lean kernel. This article is computed from the ledger — nothing here is authored; every claim carries its citation.
...
the arithmetic of geometric optics — specific consistent cases, not a full wave-optics derivation.
:::
