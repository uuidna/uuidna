...
**11 theorems**, each proven `by decide` in [lean/Astronomy.lean](/lean/Astronomy.lean), axiom-free against the bare Lean kernel. This article is computed from the ledger — nothing here is authored; every claim carries its citation.
...
the decidable arithmetic of positional astronomy — exact ratios and cycles, some (precession) classical approximations, not claims about the cosmos.
:::
