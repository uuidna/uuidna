...
They run from [enumeration_hex4_7000](/theorem/enumeration_hex4_7000) to [enumeration_hex4_7fff](/theorem/enumeration_hex4_7fff). The site builds no page per station — a page list spread into one call overflows V8's argument limit near 2^16, the span's own size — so the Worker renders each from the ledger at its sealed key (the first station is [enumeration_hex4_7000](/theorem/enumeration_hex4_7000)), and `uuidna_theorem` answers any of them.
