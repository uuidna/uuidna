...
### prose that says more than the statement OUTNUMBERS prose that repeats it — 2608 informative against 0 bare, of 2608; a doc comment identical to its own Lean statement carries nothing the proof did not already say, and this is the remaining work counted rather than a target claimed
...
### the whole prose corpus folds to ONE ℤ/9 receipt — 696008 characters across 2608 doc comments in 114 wings fold to 2; the kernel sums the per-wing character counts itself and takes the residue, the ledger's own vortex arithmetic over its own sentences, so a single changed character in any wing moves the digit
