...
### the whole prose corpus folds to ONE ℤ/9 receipt — 687775 characters across 2602 doc comments in 114 wings fold to 4; the kernel sums the per-wing character counts itself and takes the residue, the ledger's own vortex arithmetic over its own sentences, so a single changed character in any wing moves the digit
