...
```lean
(2565392635 = (71630 * (71630 - 1)) / 2) ∧ (2565392635 > 2000 * 71629)
```
...
```lean
(689 + 70861 + 75 + 5 = 71630) ∧ (80 > 0)
```

### THE FOLDERS ARE NOT ALL THE SAME THING EITHER: 70941 chunk, 437 page, 252 publication, summing to 71,630 exactly — no leaf counted twice and none left out. A crosslink graph over a store with kinds is a graph with typed nodes, which is a different and larger object than a graph over one kind; sealing the partition first is what makes that statement meaningful rather than decorative.
