...
### THE STORE AS IT STANDS, WALKED RATHER THAN QUOTED. 71,595 leaves, 71,594 parent links, 2,562,886,215 pairs available — the tree uses one link for every 35,797 pairs its own leaves already permit. Stated as an inequality with a factor rather than a percentage, because a percentage rounds and this ledger decides: the available pairs exceed the tree's links by more than two thousand times.
...
```lean
(2562886215 = (71595 * (71595 - 1)) / 2) ∧ (2562886215 > 2000 * 71594)
```
...
```lean
(681 + 70835 + 74 + 5 = 71595) ∧ (79 > 0)
```

### THE FOLDERS ARE NOT ALL THE SAME THING EITHER: 70914 chunk, 433 page, 248 publication, summing to 71,595 exactly — no leaf counted twice and none left out. A crosslink graph over a store with kinds is a graph with typed nodes, which is a different and larger object than a graph over one kind; sealing the partition first is what makes that statement meaningful rather than decorative.
