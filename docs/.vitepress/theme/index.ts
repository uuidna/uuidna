...
import './style.css'
// THE MATRIX IS A STYLESHEET, NOT A SCRIPT. Generated by gen-matrix-css from src/css.ts, so it is in the CSS
// payload at first paint and does not wait for hydration — measured 2026-09-20, every one of its 31 variables
// and its 3,060 bytes of effects reached the page only after JavaScript ran, which left the first frame, a
// reader with JavaScript off, and every server-rendered var(--seq-N) without a fallback painting nothing.
import './computed.css'
