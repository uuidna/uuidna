...
const cellsOf = (bits) => {
  ...
  for (let i = 0; i < n; i++) {
    // the line bits are NOT computed here any more: they were the binary expansion of `i`, which CSS derives
    // from :nth-child. Only `lit` depends on the address, so only `lit` is data.
    out.push({ i, lit: row[i] === 1 })
  }
  return out
}
</script>
    ...
    >
      ...
      >
        <div class="hex-board" data-slot="hex-board">
          ...
          >
            <!-- BARE SPANS ON PURPOSE. These carried a per-line class and a yang attribute, and the bit was
                 `(i >> b) & 1` — the binary expansion of the gate's OWN position, identical on every page of
                 the site. 46 KB of markup per page, about 246 MB across 5,606 pages, describing where each
                 span already was. CSS reads position with :nth-child, so the clauses in the style block below
                 (generated by src/hex-line-css.ts, checked against the arithmetic for all 128 x 6 cases) paint
                 the yang lines and the attributes are gone. -->
            <span v-for="li in hexagramBits" :key="li" />
          </div>
        </div>
      </div>
      <div
...
/* GENERATED by src/hex-line-css.ts — do not hand-edit.
 * The line values are the binary expansion of each gate's own position, so they are derived from
 * :nth-child rather than shipped as 768 per-line attributes on every page (about 246 MB across the
 * site). Default is YIN (broken); the clauses below paint the YANG lines. */
.hex-gate > span {
  display: block;
  height: 2px;
...
.hex-board > :nth-child(16n + 12) > :nth-child(4),
.hex-board > :nth-child(16n + 13) > :nth-child(4),
.hex-board > :nth-child(16n + 14) > :nth-child(4),
.hex-board > :nth-child(16n + 15) > :nth-child(4),
