...
import { theorems, PRINCIPLES } from '../../dist/theorems/index.js'
import { axiomWitness } from '../../dist/axiom-witness.js'
import { publications } from '../../dist/publish.js'
import { hexFaceOf } from '../../dist/hexagram.js'
...
import { rdRoot } from '../../dist/boundary.js'
...
/** Door stamp: handle + 32 hexbits + 16 hexagrams + occupancy + aura. One function, every monograph with an address. */
function stampFace(address) {
...
export function composeTheorem(t) {
  const face = stampFace(t.address)
...
export function composeChunk(c) {
  const face = stampFace(c.address)
  const handle = face.handle
