...
that cites another sealed key IS a citation edge, 55 prerequisites over 62 edges at this
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 1550-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 1550 theorems across 102 wings and 94 skills.
...
| Waves.lean | `Waves.lean` | waves | 17 | [`buddy_pair_squares_the_failure`](/theorem/buddy_pair_squares_the_failure) |
