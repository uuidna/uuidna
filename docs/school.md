...
that cites another sealed key IS a citation edge, 135 prerequisites over 161 edges at this
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2140-frame aura film and it was wrong. The aura IS
...
8. **Discuss the open at school** — after a wave of external research, every unverified lands in the held
   record (0 this generation) and the [open questions](/open-questions) curriculum; local labs
   (`labOf`) recompute only the sealed half. Silence never refutes ([`silence_never_refutes`](/theorem/silence_never_refutes)).
...
Every wing the ledger seals, largest first: 2140 theorems across 115 wings and 107 skills.
...
| --- | --- | --- | ---: | --- |
| The conveyor's first wave | `Wave.lean` | wave | 467 | [`a440_drift_is_heard`](/theorem/a440_drift_is_heard) |
