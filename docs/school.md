...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   ([`verify_beats_recompute_by_magnitudes`](/theorem/verify_beats_recompute_by_magnitudes)). One fold composed **117** monographs to
   ...
   **The boundary:** the same reasoning was turned on the 2617-frame aura film and it was wrong. The aura IS
...
8. **Discuss the open at school** — every lead enrolls on this page (0 held · 59 refuted · 20 refused this generation, [the leads roster](/school#leads)). Held doors also sit in [open questions](/open-questions); a refutation is a measurement, a refusal a boundary. Local labs (`labOf`) recompute only the sealed half. Silence never refutes ([`silence_never_refutes`](/theorem/silence_never_refutes)).
...
## The wings — all 117, computed from the ledger

Every wing the ledger seals, largest first: 2617 theorems across 117 wings and 109 skills.
...
| --- | --- | --- | ---: | --- |
| The conveyor's first wave | `Wave.lean` | wave | 906 | [`a440_drift_is_heard`](/theorem/a440_drift_is_heard) |
...
**79 leads** — 0 held · 59 refuted · 20 refused · 0 in trial.
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 349,278 | 6 | 58,213× | `theorems().length` |
| what does the tree hold right now? | 369,467 | 225 | 1,642× | `npm run state` |
| which Alpine domains are ported? | 1,822,410 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 81,791 | 65 | 1,258× | `leads-gate + gate-receipt --verify` |
...
| the sealed ledger | 1,397,112 | 16 | 87,319× |
