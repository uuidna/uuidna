...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   ([`verify_beats_recompute_by_magnitudes`](/theorem/verify_beats_recompute_by_magnitudes)). One fold composed **70** monographs to
   ...
   **The boundary:** the same reasoning was turned on the 1458-frame aura film and it was wrong. The aura IS
...
## The wings — all 94, computed from the ledger

Every wing the ledger seals, largest first: 1458 theorems across 94 wings and 86 skills.
