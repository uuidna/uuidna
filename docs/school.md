...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   ([`verify_beats_recompute_by_magnitudes`](/theorem/verify_beats_recompute_by_magnitudes)). One fold composed **116** monographs to
   ...
   **The boundary:** the same reasoning was turned on the 2612-frame aura film and it was wrong. The aura IS
...
## The wings — all 116, computed from the ledger

Every wing the ledger seals, largest first: 2612 theorems across 116 wings and 108 skills.
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 348,474 | 6 | 58,079× | `theorems().length` |
| what does the tree hold right now? | 368,608 | 225 | 1,638× | `npm run state` |
| which Alpine domains are ported? | 1,822,410 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 80,814 | 65 | 1,243× | `leads-gate + gate-receipt --verify` |
...
| the sealed ledger | 1,393,899 | 16 | 87,118× |
