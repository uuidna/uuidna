...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2596-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 2596 theorems across 116 wings and 108 skills.
...
| The site build as arithmetic | `SiteBuild.lean` | site-build | 7 | [`exponent_associativity_changes_the_value`](/theorem/exponent_associativity_changes_the_value) |
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 344,931 | 6 | 57,488× | `theorems().length` |
| what does the tree hold right now? | 365,038 | 225 | 1,622× | `npm run state` |
| which Alpine domains are ported? | 1,822,410 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 80,496 | 65 | 1,238× | `leads-gate + gate-receipt --verify` |
...
| the sealed ledger | 1,379,724 | 16 | 86,232× |
