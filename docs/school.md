...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   ([`verify_beats_recompute_by_magnitudes`](/theorem/verify_beats_recompute_by_magnitudes)). One fold composed **81** monographs to
   ...
   **The boundary:** the same reasoning was turned on the 1604-frame aura film and it was wrong. The aura IS
...
## The wings — all 109, computed from the ledger

Every wing the ledger seals, largest first: 1604 theorems across 109 wings and 101 skills.
