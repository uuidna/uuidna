...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2502-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 2502 theorems across 115 wings and 107 skills.
...
| --- | --- | --- | ---: | --- |
| The conveyor's first wave | `Wave.lean` | wave | 826 | [`a440_drift_is_heard`](/theorem/a440_drift_is_heard) |
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 331,204 | 6 | 55,200× | `theorems().length` |
| what does the tree hold right now? | 350,158 | 225 | 1,556× | `npm run state` |
| which Alpine domains are ported? | 1,822,378 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 77,299 | 65 | 1,189× | `leads-gate + gate-receipt --verify` |
...
| the sealed ledger | 1,324,816 | 16 | 82,801× |
| the leads record | 65,737 | 16 | 4,108× |
