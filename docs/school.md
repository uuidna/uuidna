...
that cites another sealed key IS a citation edge, 73 prerequisites over 83 edges at this
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 1622-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 1622 theorems across 110 wings and 102 skills.
...
| The conveyor's first wave | `Wave.lean` | wave | 17 | [`a440_not_on_the_vortex`](/theorem/a440_not_on_the_vortex) |
