...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2658-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 2658 theorems across 118 wings and 110 skills.
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 357,521 | 6 | 59,586× | `theorems().length` |
| what does the tree hold right now? | 378,079 | 225 | 1,680× | `npm run state` |
| which Alpine domains are ported? | 1,822,417 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 83,445 | 65 | 1,283× | `leads-gate + gate-receipt --verify` |
...
| the sealed ledger | 1,430,085 | 16 | 89,380× |
