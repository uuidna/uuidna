...
that cites another sealed key IS a citation edge, 199 prerequisites over 259 edges at this
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 71018-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 71018 theorems across 252 wings and 118 skills.
...
| The quantum computer | `Quantum.lean` | quantum | 64 | [`all_signaling_duality`](/theorem/all_signaling_duality) |
...
| The Glagolitic numerals & Pliska rosette | `Glagolitic.lean` | glagolitic | 13 | [`abjad_four_ranks`](/theorem/abjad_four_ranks) |
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 5,850,969 | 6 | 975,161× | `theorems().length` |
| what does the tree hold right now? | 5,873,806 | 225 | 26,105× | `npm run state` |
...
Median ratio **26,105×**. Tokens are estimated at four bytes each — an approximation, and
...
| the sealed ledger | 23,403,877 | 16 | 1,462,742× |
...
- **holds** · Generate all only from Lean — the sealed theorems are the single source; the derived layer is computed and diff-gated.
  <br><small>enforced by: conformance:single-source-ledger + the pre-push git-diff — every one of 71018 theorems is sourced from a lean/*.lean file</small>
- **holds** · Any manual fails — every theorem recomputes its content-address; a hand-tampered theorem turns the recompute test red.
  <br><small>enforced by: conformance:ledger-dna-recomputes + recompute.test — all 71018 addresses recompute; 0 forged</small>
