...
that cites another sealed key IS a citation edge, 73 prerequisites over 82 edges at this
...
| [`window_not_universal`](/theorem/window_not_universal) | 2 | [`cubes_sum_to_square_of_triangle`](/theorem/cubes_sum_to_square_of_triangle), [`team_pairs_triples_cover`](/theorem/team_pairs_triples_cover) |
...
| [`moduli_waste_states`](/theorem/moduli_waste_states) | 1 | [`door_of_the_referrer`](/theorem/door_of_the_referrer) |
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   ([`verify_beats_recompute_by_magnitudes`](/theorem/verify_beats_recompute_by_magnitudes)). One fold composed **82** monographs to
   ...
   **The boundary:** the same reasoning was turned on the 1611-frame aura film and it was wrong. The aura IS
...
## The wings — all 110, computed from the ledger

Every wing the ledger seals, largest first: 1611 theorems across 110 wings and 102 skills.
...
| The conveyor's first wave | `Wave.lean` | wave | 6 | [`a440_not_on_the_vortex`](/theorem/a440_not_on_the_vortex) |
