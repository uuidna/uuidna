...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2163-frame aura film and it was wrong. The aura IS
...
8. **Discuss the open at school** — every lead enrolls on this page (6 held · 47 refuted · 20 refused this generation, [the leads roster](/school#leads)). Held doors also sit in [open questions](/open-questions); a refutation is a measurement, a refusal a boundary. Local labs (`labOf`) recompute only the sealed half. Silence never refutes ([`silence_never_refutes`](/theorem/silence_never_refutes)).
...
Every wing the ledger seals, largest first: 2163 theorems across 115 wings and 107 skills.
...
| --- | --- | --- | ---: | --- |
| The conveyor's first wave | `Wave.lean` | wave | 487 | [`a440_drift_is_heard`](/theorem/a440_drift_is_heard) |
...
**73 leads** — 6 held · 47 refuted · 20 refused · 6 in trial.
...
- **held** · `9d04ded9` — one census() costs 66 seconds — it evaluates every sealed statement under an involution, about 30 ms per theorem. Memoising removed all REDUNDANCY (four calls became two, one per involution, the minimum), so the suite fl…
  <br><small>a measurement of where those 30 ms go inside involutionSurvives — parse, evaluate, or address fold — and then either a cheaper evaluator or a statement that deciding 2,163 statements twice is what the guarantee costs</small>
