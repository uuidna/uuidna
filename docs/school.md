...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2626-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 2626 theorems across 118 wings and 110 skills.
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 352,083 | 6 | 58,680× | `theorems().length` |
| what does the tree hold right now? | 372,273 | 225 | 1,654× | `npm run state` |
| which Alpine domains are ported? | 1,822,410 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 82,127 | 65 | 1,263× | `leads-gate + gate-receipt --verify` |
...
| the sealed ledger | 1,408,334 | 16 | 88,020× |
