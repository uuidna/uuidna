...
that cites another sealed key IS a citation edge, 54 prerequisites over 61 edges at this
...
| [`rosette_quantum_doubling_is_two_coins`](/theorem/rosette_quantum_doubling_is_two_coins) | 1 | [`a_handle_is_eight_pairs_paid_it_is_sixteen`](/theorem/a_handle_is_eight_pairs_paid_it_is_sixteen) |
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   ([`verify_beats_recompute_by_magnitudes`](/theorem/verify_beats_recompute_by_magnitudes)). One fold composed **74** monographs to
   ...
   **The boundary:** the same reasoning was turned on the 1549-frame aura film and it was wrong. The aura IS
...
## The wings — all 102, computed from the ledger

Every wing the ledger seals, largest first: 1549 theorems across 102 wings and 94 skills.
...
| Waves.lean | `Waves.lean` | waves | 16 | [`buddy_pair_squares_the_failure`](/theorem/buddy_pair_squares_the_failure) |
