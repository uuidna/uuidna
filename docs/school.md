...
that cites another sealed key IS a citation edge, 137 prerequisites over 165 edges at this
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   ([`verify_beats_recompute_by_magnitudes`](/theorem/verify_beats_recompute_by_magnitudes)). One fold composed **115** monographs to
   ...
   **The boundary:** the same reasoning was turned on the 2515-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 2515 theorems across 115 wings and 107 skills.
...
| --- | --- | --- | ---: | --- |
| The conveyor's first wave | `Wave.lean` | wave | 839 | [`a440_drift_is_heard`](/theorem/a440_drift_is_heard) |
...
| The sequence & reflection group | `Sequence.lean` | sequence | 32 | [`agl_order_54`](/theorem/agl_order_54) |
...
| The fused ring | `Crt.lean` | crt | 10 | [`axes_stride_coprime`](/theorem/axes_stride_coprime) |
...
- **refuted** · `cbbc6b32` — A negation in the comment above a Lean line must be discharged by the line below it. Run by hand over the newest wing it convicted 6 of 12; the surviving 4 are SCOPE-declared, proven, or scanner artefacts ('non-covering'…
  <br><small>PAID the enforceable half. leanNegationGaps() (one-receipt lean-negation + state finder) flags object-level negation comments without ≠/¬/!= discharge, exempts SCOPE:/HONEST SCOPE:, and documents the split VectorEquilibrium.lean already states. NOT a blocking guard: first run flo…</small>
...
- **refused** · `a56e35a0` — Honesty / HONEST SCOPE fields, covering hunts, or includes() disclaimer tests on new constructors.
  <br><small>Honesty is the court (by decide keys, docs/doctrine.md), not TypeScript. Captain 2026-08-30: strip existing honest: payloads from constructors on touch; no new HONEST SCOPE fields, covering hunts, or includes() disclaimer tests. Scope lives in doctrine and sealed keys; R&D contin…</small>
...
## The port — 25 domains, 7 of them carrying an API

Alpine publishes; uuidna counts and, where it has something of its own to offer, answers. 25 domains
are censused from the committed mirror and 7 carry one API each — 3164 packages behind those seven doors.
...
Ported API receipt `942f6b3d-d1fd-8c2f-a854-4fa5fa145f9b`. Package counts are per domain and the domains OVERLAP — a chat bridge is
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 333,151 | 6 | 55,525× | `theorems().length` |
| what does the tree hold right now? | 353,249 | 225 | 1,569× | `npm run state` |
| which Alpine domains are ported? | 1,822,378 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 78,754 | 65 | 1,211× | `leads-gate + gate-receipt --verify` |
...
| the sealed ledger | 1,332,607 | 16 | 83,287× |
| the leads record | 70,309 | 16 | 4,394× |
