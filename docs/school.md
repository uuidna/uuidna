...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2518-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 2518 theorems across 115 wings and 107 skills.
...
| --- | --- | --- | ---: | --- |
| The conveyor's first wave | `Wave.lean` | wave | 842 | [`a440_drift_is_heard`](/theorem/a440_drift_is_heard) |
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 333,097 | 6 | 55,516× | `theorems().length` |
| what does the tree hold right now? | 353,186 | 225 | 1,569× | `npm run state` |
| which Alpine domains are ported? | 1,822,378 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 78,829 | 65 | 1,212× | `leads-gate + gate-receipt --verify` |
...
| the sealed ledger | 1,332,390 | 16 | 83,274× |
