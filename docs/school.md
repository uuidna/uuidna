...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2588-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 2588 theorems across 115 wings and 107 skills.
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 343,875 | 6 | 57,312× | `theorems().length` |
| what does the tree hold right now? | 363,963 | 225 | 1,617× | `npm run state` |
| which Alpine domains are ported? | 1,822,410 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 80,411 | 65 | 1,237× | `leads-gate + gate-receipt --verify` |
...
| the sealed ledger | 1,375,500 | 16 | 85,968× |
