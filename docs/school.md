...
that cites another sealed key IS a citation edge, 121 prerequisites over 143 edges at this
...
| [`axes_stride_coprime`](/theorem/axes_stride_coprime) | 1 | [`stride_cycle_is_modulus_over_gcd`](/theorem/stride_cycle_is_modulus_over_gcd) |
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2120-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 2120 theorems across 115 wings and 107 skills.
