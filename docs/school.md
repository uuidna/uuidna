...
that cites another sealed key IS a citation edge, 75 prerequisites over 87 edges at this
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   ([`verify_beats_recompute_by_magnitudes`](/theorem/verify_beats_recompute_by_magnitudes)). One fold composed **83** monographs to
   ...
   **The boundary:** the same reasoning was turned on the 1639-frame aura film and it was wrong. The aura IS
...
## The wings — all 111, computed from the ledger

Every wing the ledger seals, largest first: 1639 theorems across 111 wings and 103 skills.
...
| The conveyor's first wave | `Wave.lean` | wave | 28 | [`a440_not_on_the_vortex`](/theorem/a440_not_on_the_vortex) |
