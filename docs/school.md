...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2653-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 2653 theorems across 118 wings and 110 skills.
...
| --- | --- | --- | ---: | --- |
| The conveyor's first wave | `Wave.lean` | wave | 933 | [`a440_drift_is_heard`](/theorem/a440_drift_is_heard) |
...
| The hardware-verifiable binary algebra | `Hardware.lean` | hardware | 17 | [`and_gate_truth_table`](/theorem/and_gate_truth_table) |
...
| what does the tree hold right now? | 375,638 | 225 | 1,669× | `npm run state` |
| which Alpine domains are ported? | 1,822,410 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 82,940 | 65 | 1,276× | `leads-gate + gate-receipt --verify` |
