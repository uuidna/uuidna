...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   ([`verify_beats_recompute_by_magnitudes`](/theorem/verify_beats_recompute_by_magnitudes)). One fold composed **80** monographs to
   ...
   **The boundary:** the same reasoning was turned on the 1598-frame aura film and it was wrong. The aura IS
...
## The wings — all 108, computed from the ledger

Every wing the ledger seals, largest first: 1598 theorems across 108 wings and 100 skills.
