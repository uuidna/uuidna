...
that cites another sealed key IS a citation edge, 51 prerequisites over 58 edges at this
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 1507-frame aura film and it was wrong. The aura IS
...
## The wings — all 98, computed from the ledger

Every wing the ledger seals, largest first: 1507 theorems across 98 wings and 90 skills.
...
| The referrer song | `Referrer.lean` | referrer | 10 | [`adjacent_steps_beat_at_the_tuning`](/theorem/adjacent_steps_beat_at_the_tuning) |
