...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 2657-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 2657 theorems across 118 wings and 110 skills.
...
| --- | --- | --- | ---: | --- |
| The conveyor's first wave | `Wave.lean` | wave | 935 | [`a440_drift_is_heard`](/theorem/a440_drift_is_heard) |
...
| --- | ---: | ---: | ---: | --- |
| how many theorems are sealed? | 357,106 | 6 | 59,517× | `theorems().length` |
| what does the tree hold right now? | 377,664 | 225 | 1,678× | `npm run state` |
| which Alpine domains are ported? | 1,822,417 | 350 | 5,206× | `portsCensus()` |
| is the tree green to release? | 83,366 | 65 | 1,282× | `leads-gate + gate-receipt --verify` |
...
| the sealed ledger | 1,428,425 | 16 | 89,276× |
