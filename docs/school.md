...
that cites another sealed key IS a citation edge, 84 prerequisites over 96 edges at this
...
7. **Seal the input, never its pure function — and know where that stops** — a digest of f(x) moves exactly when
   ...
   **The boundary:** the same reasoning was turned on the 1662-frame aura film and it was wrong. The aura IS
...
Every wing the ledger seals, largest first: 1662 theorems across 112 wings and 104 skills.
...
| The conveyor's first wave | `Wave.lean` | wave | 45 | [`a440_drift_is_heard`](/theorem/a440_drift_is_heard) |
