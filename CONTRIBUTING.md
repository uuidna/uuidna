...
The pre-push hook runs the hexbit-fast gate (`next --verify`) and **blocks a commit push** on any denial. That
path is **not** a version cut. The **publish source of truth is `npm run gate-all`** (`prepublishOnly`, and
`npm run release-cut` before a tag). Hexbit-fast saying ready does not mean npm will accept the package. If the
