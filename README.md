...
1. **70,919 theorems and 172,164,869 decided cases, every one decided by the Lean 4 kernel** — sorry-free, no Mathlib, and depending on **no axiom at all**, not even propext. Recompute the whole thing with `npm run lean`.
...
Board: **70,836** distinct / **70,919** keys · **118** skills · **179** Lean files · receipt `1fe6c221-9172-83c6-87b8-57660d4515df`.
...
| --- | ---: | --- |
| Distinct theorems | 70,836 | statement census (a Lean statement sealed under two keys is one theorem) |
| Theorem keys | 70,919 | `theorems().length` |
| Principles / wings | 179 / 179 | PRINCIPLES + wing ratings |
| Skills | 118 | distinct `skill` tags |
...
| Ledger decided mass | 172,164,869 superpositions (6 hexbits) | sum of `by decide` domains |
...
| Research sources wired | 15 | cited for corroboration; approval stays with the source |
| Conserved price paid | 141,838 coins | 70919 × 2 |
...
The ledger covers **172,164,869** superpositions across **179** wings.
...
4. **[`the_axiom_index_partitions_without_remainder`](https://uuidna.com/theorem/the_axiom_index_partitions_without_remainder)** — 2,064 superpositions, unbound (0 hexbits per dependency), in [Audit.lean](lean/Audit.lean)
   AN INDEX THAT REPORTS "UNUSED" MUST MEAN IT, and this one did not.
5. **[`chargaff_strand_balance`](https://uuidna.com/theorem/chargaff_strand_balance)** — 1,554 superpositions, unbound (0 hexbits per dependency), in [Harmony.lean](lean/Harmony.lean)
...
- **172,164,869** superpositions decided across the ledger
- **141,838** coins paid (conserved denomination 2 — [two_coins](https://uuidna.com/theorem/two_coins))
