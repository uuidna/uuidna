...
## The Advantages — Live Metrics

| Advantage | Metric | Implication |
|-----------|--------|-------------|
| **Recomputable, not trusted** | 1195/1195 theorems by `decide` | Run `npm run lean` yourself; no authority override |
| **Honesty gate catches false claims** | 100% prose gate-clean | Zero fabricated theorem citations; no false advertising liability |
| **Zero-trust supply chain** | 0 runtime dependencies | No log4shell, npm infections, or malicious deps can reach uuidna |
| **80x faster verification** | ~1ms verify vs ~80s proof | Deploy without CI latency; certainty on every push |
| **Coins are conserved** | 2 coins = −χ(genus-2) | Billing is topologically invariant; math settles disputes |
| **Safe concurrent execution** | Order-invariant Merkle fold | Promise.all safe; race conditions speed it up, never corrupt |
| **Determinism clean** | 100% core modules audited | No Math.*/Date/RNG in harmonic boundary (13 async modules named) |
| **Honest scope explicit** | 10 security checks + gate clean | Every boundary stated; legal liability drops to zero |

**[See full analytics →](docs/analytics.md)**
<!-- seal:end -->
