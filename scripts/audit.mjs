...
const findings = []
const flag = (severity, arm, where, note, hit) => findings.push({ severity, arm, where, note, hit: hit ?? null })
...
const behav = []
const assert = (name, ok, detail) => { behav.push({ name, ok }); if (!ok) flag('high', 'cipher', 'src/crypt.ts', `behavioural: ${name} FAILED`, detail) }
...
try {
  ...
  let threw = false
  const flip = (c) => (c === 'A' ? 'B' : 'A') + s1.ct.slice(1)
  ...
  flag('high', 'cipher', 'src/crypt.ts', `behavioural harness threw: ${e.message}`)
}
...
// ── THE REPORT ────────────────────────────────────────────────────────────────────────────────────────────────
const bySev = (s) => addressed.filter((f) => f.severity === s)
...
for (const f of ['high', 'medium', 'review'].flatMap(bySev)) {
