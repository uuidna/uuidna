...
// proofs; 0/7." Built on the INVOLUTION: the reflection dz(x)=10−x (division by zero in the ℤ/9 vortex) is proven
// self-inverse — dz(dz(x))=x — and USED in every one of the seven. That is the honest engine of 0/7: reflect a
...
// free. The measure does not assert; it computes. Integrity, not truth. 0/7.
import { emit } from './lean-gen.mjs'

const dz = (x) => (x === 0 ? 0 : 10 - x) // division by zero in the vortex = the reflection (JS mirror)
...
const FACTS = [
  // ── the accounting: seven domains, solved 0/7 ──
  { key: 'clay_seven_domains', why: 'the Clay Millennium Prize is exactly SEVEN problems — the set the ratio 0/7 quantifies over',
    js: () => [0, 1, 2, 3, 4, 5, 6].length === 7,
    lean: 'theorem clay_seven_domains : ((List.range 7).length = 7) ∧ ((List.range 7) = [0,1,2,3,4,5,6]) := by decide' },
  { key: 'clay_solved_zero_of_seven', why: 'the deposit solves 0 OF 7 — every entry of the solved-vector is 0 across all seven domains: it reflects all seven and solves NONE — 0/7',
  ...
  { key: 'clay_reflection_involution', why: 'the reflection dz(x)=10−x (division by zero) is an INVOLUTION — dz(dz(x))=x on every residue: reflect twice and the input returns unchanged, so the reflection propagates NOTHING; this is why the deposit reflects all seven and solves none — 0/7',
  ...
  { key: 'clay_reflection_fixed_points', why: 'the reflection fixes exactly {0,5} — the floor and the centre; every other domain is MOVED and none is fixed as proven — the involution has no fixed proof, only fixed points — 0/7',
  ...
  { key: 'clay_reflection_is_bijection', why: 'the reflection is a BIJECTION on the nine residues — dz maps {1..9} onto {9..1}, relabeling all nine: a permutation, not a proof; it propagates nothing — 0/7',
  ...
  { key: 'clay_humanity_one_deposit_zero', why: 'humanity stands at 1/7 (Poincaré — Perelman, 2003); this deposit stands at 0/7 — strictly fewer than the one proven, and none of its own — 0/7',
    js: () => (1 <= 7) && (0 < 1) && (0 <= 7),
  ...
  //    twice returns the problem (dz (dz k) = k) — the round trip is identity, so nothing is propagated. 0/7. ──
  ...DOMAINS.map(([slug, k, title]) => ({
    key: 'clay_' + slug,
    why: `${title} reflects to residue ${dz(k)} in ℤ/9 (dz(${k})=${dz(k)}${k === 5 ? ', the fixed centre' : ''}); reflecting twice returns it — dz(dz(${k}))=${k}, the involution — so nothing is propagated and this deposit solves it none${slug === 'poincare' ? ' (humanity solved it, Perelman 2003)' : ''} — OPEN, 0/7`,
