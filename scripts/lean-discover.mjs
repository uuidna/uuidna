...
const BASE = 9
const m = (n) => ((n % BASE) + BASE) % BASE
const gcd = (a, b) => (b === 0 ? a : gcd(b, a % b))
const R = Array.from({ length: BASE }, (_, i) => i) // 0 .. BASE-1, computed
const hasInverse = (a) => R.some((e) => m(a * e) === 1)
const inverseOf = (a) => R.find((e) => m(a * e) === 1)
const orbit2 = () => { const o = []; let x = 1; do { o.push(x); x = m(x * 2) } while (x !== 1); return o } // ⟨2⟩ by iteration
const pow = (a, k) => { let r = 1; for (let i = 0; i < k; i++) r = m(r * a); return r }
...
// 1) compute — every discovered fact must hold in JS
const jsFail = DISCOVERED.filter((f) => f.js() !== true)
if (jsFail.length) { console.log('✗ discovery failed in JS: ' + jsFail.map((f) => f.key).join(', ')); process.exit(1) }
console.log('self-discovered ' + DISCOVERED.length + ' ℤ/9 facts by function (no hardcoded arrays) — all hold in JS.')

// 2) which are NEW? compare against the existing lean/*.lean files
...
const isNew = (key) => !new RegExp('\\b' + key + '\\b').test(existing)
...
// 3) generate — a self-computing Lean file: invB DERIVES the units, the theorems recompute each property
const lean = `-- lean/Discover.lean — GENERATED by scripts/lean-discover.mjs (self-discovery; all computes by itself). No
-- hardcoded structure: \`invB a\` DERIVES whether a is a unit (it has an inverse), and every theorem RECOMPUTES its
-- property by filter/any/all over the ring — nothing typed as a static list. Every proof is \`by decide\`, sorry-free.
-- Regenerate with \`npm run lean:discover\`. Integrity, not truth. 0/7.

def invB (a : Nat) : Bool := (List.range 9).any (fun e => (a * e) % 9 == 1)   -- a is a unit iff it has an inverse

${DISCOVERED.map((f) => '-- ' + f.why + '\n' + f.lean).join('\n\n')}
`
const path = join(ROOT, 'lean', 'Discover.lean')
writeFileSync(path, lean)
console.log('generated ' + DISCOVERED.length + ' Lean theorems → lean/Discover.lean')

// 4) verify
try {
  execSync('lean ' + JSON.stringify(path), { cwd: ROOT, stdio: 'pipe' })
  console.log('✓ VERIFIED: lean/Discover.lean compiles sorry-free — ' + DISCOVERED.length + ' self-discovered theorems proved.')
} catch (e) {
